...
class WebSocketRouteDispatcher extends _dispatcher.Dispatcher {
  constructor(scope, id, url, frame) {
    ...
    this._eventListeners.push(
      ...
      _eventsHelper.eventsHelper.addEventListener(
        frame._page,
        _page.Page.Events.FrameDetached,
        (frame) => {
          if (frame === this._frame) this._executionContextGone();
        },
      ),
      _eventsHelper.eventsHelper.addEventListener(frame._page, _page.Page.Events.Close, () =>
        this._executionContextGone(),
      ),
      _eventsHelper.eventsHelper.addEventListener(frame._page, _page.Page.Events.Crash, () =>
        this._executionContextGone(),
      ),
    );
    ...
    if (!context[kBindingInstalledSymbol]) {
      ...
      await context.exposeBinding("__pwWebSocketBinding", false, (source, payload) => {
        if (payload.type === "onCreate") {
          const pageDispatcher = _pageDispatcher.PageDispatcher.fromNullable(
            contextDispatcher,
            source.page,
          );
          let scope;
          if (
            pageDispatcher &&
            matchesPattern(pageDispatcher, context._options.baseURL, payload.url)
          )
          ...
          if (scope) {
            ...
            source.frame
              .evaluateExpression(`globalThis.__pwWebSocketDispatch(${JSON.stringify(request)})`)
              .catch(() => {});
          }
          return;
        }
        ...
        if (payload.type === "onMessageFromPage")
          dispatcher === null ||
            dispatcher === void 0 ||
            dispatcher._dispatchEvent("messageFromPage", {
        ...
        if (payload.type === "onMessageFromServer")
          dispatcher === null ||
            dispatcher === void 0 ||
            dispatcher._dispatchEvent("messageFromServer", {
              message: payload.data.data,
              isBase64: payload.data.isBase64,
            });
        if (payload.type === "onClosePage")
          dispatcher === null ||
            dispatcher === void 0 ||
            dispatcher._dispatchEvent("closePage", {
              code: payload.code,
        ...
        if (payload.type === "onCloseServer")
          dispatcher === null ||
            dispatcher === void 0 ||
            dispatcher._dispatchEvent("closeServer", {
              code: payload.code,
              reason: payload.reason,
              wasClean: payload.wasClean,
            });
...
function matchesPattern(dispatcher, baseURL, url) {
  for (const pattern of dispatcher._webSocketInterceptionPatterns || []) {
    const urlMatch = pattern.regexSource
      ? new RegExp(pattern.regexSource, pattern.regexFlags)
      : pattern.glob;
