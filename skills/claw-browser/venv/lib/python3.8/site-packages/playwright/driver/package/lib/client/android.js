...
class AndroidDevice extends _channelOwner.ChannelOwner {
  ...
  async shell(command) {
    const { result } = await this._channel.shell({
      command,
    });
    return result;
  }
...
class AndroidSocket extends _channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
    this._channel.on("data", ({ data }) => this.emit(_events.Events.AndroidSocket.Data, data));
...
class AndroidWebView extends _events2.EventEmitter {
  ...
  async _fetchPage() {
    const { context } = await this._device._channel.connectToWebView({
