...
class ElementHandle extends _jsHandle.JSHandle {
  ...
  async getAttribute(name) {
    const value = (
      await this._elementChannel.getAttribute({
        name,
      })
    ).value;
  ...
  async $(selector) {
    return ElementHandle.fromNullable(
      (
        await this._elementChannel.querySelector({
          selector,
        })
      ).element,
    );
...
function filePayloadExceedsSizeLimit(payloads) {
  return (
    ...
    _fileUtils.fileUploadSizeLimit
  );
}
async function resolvePathsAndDirectoryForInputFiles(items) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      ...
      var _localPaths;
      (_localPaths = localPaths) !== null && _localPaths !== void 0
        ? _localPaths
        : (localPaths = []);
  ...
  if (items.some((item) => typeof item === "string")) {
    ...
    if (context._connection.isRemote()) {
      const files = localDirectory
        ? (
            await _fs.default.promises.readdir(localDirectory, {
              withFileTypes: true,
              recursive: true,
            })
          )
            .filter((f) => f.isFile())
            .map((f) => _path.default.join(f.path, f.name))
        : localPaths;
          ...
          context._channel.createTempFiles({
            ...
            items: await Promise.all(
              files.map(async (file) => {
                ...
                return {
                  name: localDirectory
                    ? _path.default.relative(localDirectory, file)
