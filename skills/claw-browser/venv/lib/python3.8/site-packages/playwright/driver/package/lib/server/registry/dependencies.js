...
const languageBindingVersion =
  process.env.PW_CLI_DISPLAY_VERSION || require("../../../package.json").version;
...
async function writeDockerVersion(dockerImageNameTemplate) {
  ...
  await _fs.default.promises.writeFile(
    dockerVersionFilePath,
    JSON.stringify(dockerVersion(dockerImageNameTemplate), null, 2),
    "utf8",
  );
...
async function installDependenciesLinux(targets, dryRun) {
  ...
  if (!_hostPlatform.isOfficiallySupportedPlatform)
    console.warn(
      `BEWARE: your OS is not officially supported by Playwright; installing dependencies for ${platform} as a fallback.`,
  ...
  await new Promise((resolve, reject) => {
    child.on("exit", (code) =>
      code === 0 ? resolve() : reject(new Error(`Installation process exited with code: ${code}`)),
    );
...
async function validateDependenciesLinux(sdkLanguage, linuxLddDirectories, dlOpenLibraries) {
  ...
  for (const directoryPath of directoryPaths)
    lddPaths.push(...(await executablesOrSharedLibraries(directoryPath)));
  const missingDepsPerFile = await Promise.all(
  ...
  const libraryToPackageNameMapping = _nativeDeps.deps[_hostPlatform.hostPlatform]
    ? {
        ...(((_deps$hostPlatform = _nativeDeps.deps[_hostPlatform.hostPlatform]) === null ||
        _deps$hostPlatform === void 0
          ? void 0
          : _deps$hostPlatform.lib2package) || {}),
  ...
  const maybeSudo =
    (_process$getuid = (_process = process).getuid) !== null &&
    _process$getuid !== void 0 &&
    _process$getuid.call(_process) &&
    os.platform() !== "win32"
      ? "sudo "
      : "";
  ...
  if (
    dockerInfo &&
    !dockerInfo.driverVersion.startsWith(
      (0, _userAgent.getPlaywrightVersion)(true /* majorMinorOnly */) + ".",
    )
  ) {
    ...
    const requiredDockerImage = dockerInfo.dockerImageName.replace(
    ...
    errorLines.push(
      ...[
        ...
        ``,
        `    ${maybeSudo}${(0, _.buildPlaywrightCLICommand)(sdkLanguage, "install-deps")}`,
        ``,
...
async function executablesOrSharedLibraries(directoryPath) {
  ...
  const allPaths = (await _fs.default.promises.readdir(directoryPath)).map((file) =>
    _path.default.resolve(directoryPath, file),
  );
...
async function missingFileDependenciesWindows(filePath) {
  ...
  const { stdout, code } = await (0, _spawnAsync.spawnAsync)(executable, [filePath], {
    cwd: dirname,
    env: {
      ...process.env,
...
async function missingFileDependencies(filePath, extraLDPaths) {
  ...
  const { stdout, code } = await (0, _spawnAsync.spawnAsync)("ldd", [filePath], {
    cwd: dirname,
    env: {
      ...process.env,
      LD_LIBRARY_PATH,
...
async function missingDLOPENLibraries(libraries) {
  ...
  const { stdout, code, error } = await (0, _spawnAsync.spawnAsync)("/sbin/ldconfig", ["-p"], {});
...
async function transformCommandsForRoot(commands) {
  ...
  const isRoot =
    ((_process$getuid2 = (_process2 = process).getuid) === null || _process$getuid2 === void 0
      ? void 0
      : _process$getuid2.call(_process2)) === 0;
  if (isRoot)
    return {
      command: "sh",
