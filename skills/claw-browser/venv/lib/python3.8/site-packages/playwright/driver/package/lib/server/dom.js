  ...
  injectedScript() {
    if (!this._injectedScriptPromise) {
      ...
      this._injectedScriptPromise = this.rawEvaluateHandle(source).then(
        (objectId) => new js.JSHandle(this, "object", "InjectedScript", objectId),
      );
    }
...
class ElementHandle extends js.JSHandle {
  ...
  async _initializePreview() {
    ...
    this._setPreview(
      await utility.evaluate((injected, e) => "JSHandle@" + injected.previewNode(e), this),
    );
  }
  asElement() {
    return this;
  ...
  async isIframeElement() {
    return this.evaluateInUtility(
      ([injected, node]) => node && (node.nodeName === "IFRAME" || node.nodeName === "FRAME"),
      {},
    );
  }
  async contentFrame() {
  ...
  async _waitAndScrollIntoViewIfNeeded(progress, waitForVisible) {
    const result = await this._retryAction(
      progress,
      "scroll into view",
      async () => {
        progress.log(`  waiting for element to be stable`);
        const waitResult = await this.evaluateInUtility(
          async ([injected, node, { waitForVisible }]) => {
            return await injected.checkElementStates(
              node,
  ...
  async _offsetPoint(offset) {
    const [box, border] = await Promise.all([
      this.boundingBox(),
      this.evaluateInUtility(([injected, node]) => injected.getElementBorderWidth(node), {}).catch(
        (e) => {},
      ),
    ]);
  ...
  async _retryAction(progress, actionName, action, options) {
    ...
    while (progress.isRunning()) {
      if (retry) {
        progress.log(
          `retrying ${actionName} action${options.trial ? " (trial run)" : ""}, attempt #${retry}`,
        );
        const timeout = waitTime[Math.min(retry - 1, waitTime.length - 1)];
        if (timeout) {
          progress.log(`  waiting ${timeout}ms`);
          const result = await this.evaluateInUtility(
            ([injected, node, timeout]) => new Promise((f) => setTimeout(f, timeout)),
            timeout,
          );
  ...
  async _retryPointerAction(progress, actionName, waitForEnabled, action, options) {
    ...
    return await this._retryAction(
      progress,
      actionName,
      async (retry) => {
        ...
        return await this._performPointerAction(
          progress,
          actionName,
          waitForEnabled,
          action,
  ...
  async _performPointerAction(
    ...
    if (!force) {
      const elementStates = waitForEnabled
        ...
        : ["visible", "stable"];
      progress.log(
        `  waiting for element to be ${waitForEnabled ? "visible, enabled and stable" : "visible and stable"}`,
      );
      const result = await this.evaluateInUtility(
        async ([injected, node, { elementStates }]) => {
      ...
      progress.log(
        `  element is ${waitForEnabled ? "visible, enabled and stable" : "visible and stable"}`,
      );
    }
    ...
    if (force) {
      ...
      const actionType =
        actionName === "move and up"
          ? "drag"
          : actionName === "hover" || actionName === "tap"
            ? actionName
            : "mouse";
      const handle = await this.evaluateHandleInUtility(
    ...
    const actionResult = await this._page._frameManager.waitForSignalsCreatedBy(
      progress,
      options.waitAfter === true,
      async () => {
        ...
        if (hitTargetInterceptionHandle) {
          const stopHitTargetInterception = this._frame
            .raceAgainstEvaluationStallingEvents(() => {
              return hitTargetInterceptionHandle.evaluate((h) => h.stop());
            })
            .catch((e) => "done")
            .finally(() => {
              var _hitTargetInterceptio;
              (_hitTargetInterceptio = hitTargetInterceptionHandle) === null ||
                _hitTargetInterceptio === void 0 ||
                _hitTargetInterceptio.dispose();
            });
  ...
  async _selectOption(progress, elements, values, options) {
    ...
    await this._retryAction(
      ...
      async () => {
        ...
        const result = await this.evaluateInUtility(
          async ([injected, node, { optionsToSelect, force }]) => {
            if (!force) {
  ...
  async _fill(progress, value, options) {
    ...
    return await this._retryAction(
      ...
      async () => {
        ...
        const result = await this.evaluateInUtility(
          async ([injected, node, { value, force }]) => {
            if (!force) {
  ...
  async _setInputFiles(progress, items) {
    ...
    const multiple =
      (filePayloads && filePayloads.length > 1) || (localPaths && localPaths.length > 1);
    const result = await this.evaluateHandleInUtility(
      ([injected, node, { multiple, directoryUpload }]) => {
    ...
    if (localPaths || localDirectory) {
      ...
      await Promise.all(
        localPathsOrDirectory.map((localPath) =>
          _fs.default.promises.access(localPath, _fs.default.constants.F_OK),
        ),
      );
      // Browsers traverse the given directory asynchronously and we want to ensure all files are uploaded.
      const waitForInputEvent = localDirectory
        ? this.evaluate(
            (node) =>
              new Promise((fulfill) => {
  ...
  async _checkFrameIsHitTarget(point) {
    ...
    while (frame.parentFrame()) {
      ...
      const style = await frameElement
        .evaluateInUtility(([injected, iframe]) => injected.describeIFrameStyle(iframe), {})
        .catch((e) => "error:notconnected");
...
const kUnableToAdoptErrorMessage = (exports.kUnableToAdoptErrorMessage =
