...
class Screenshotter {
  ...
  async screenshotElement(progress, handle, options) {
    ...
    return this._queue.postTask(async () => {
      ...
      const fitsViewport =
        boundingBox.width <= viewportSize.width && boundingBox.height <= viewportSize.height;
  ...
  async _preparePageForScreenshot(progress, frame, screenshotStyle, hideCaret, disableAnimations) {
    ...
    await this._page.safeNonStallingEvaluateInAllFrames(
      "(" +
        inPagePrepareForScreenshots.toString() +
        `)(${JSON.stringify(screenshotStyle)}, ${hideCaret}, ${disableAnimations}, ${syncAnimations})`,
      "utility",
    );
    if (!process.env.PW_TEST_SCREENSHOT_NO_FONTS_READY) {
      progress.log("waiting for fonts to load...");
      await frame
        .nonStallingEvaluateInExistingContext("document.fonts.ready", "utility")
        .catch(() => {});
  ...
  async _screenshot(progress, format, documentRect, viewportRect, fitsViewport, options) {
    ...
    const quality =
      format === "jpeg"
        ? (_options$quality = options.quality) !== null && _options$quality !== void 0
          ? _options$quality
          : 80
        : undefined;
