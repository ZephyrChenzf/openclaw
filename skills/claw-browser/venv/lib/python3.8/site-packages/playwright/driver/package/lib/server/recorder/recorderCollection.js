...
class RecorderCollection extends _events.EventEmitter {
  ...
  async _addAction(actionInContext, callback) {
    ...
    const { callMetadata, mainFrame } = (0, _recorderUtils.callMetadataForAction)(
      this._pageAliases,
      actionInContext,
    );
    ...
    this._actions.push(actionInContext);
    this._fireChange();
    const error = await (callback === null || callback === void 0
      ? void 0
      : callback(callMetadata).catch((e) => e));
    ...
    mainFrame.instrumentation
      .onAfterCall(mainFrame, callMetadata)
      .then(() => {
        this._fireChange();
      })
      .catch(() => {});
