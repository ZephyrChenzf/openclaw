...
function validateTraceUrls(traceUrls) {
  for (const traceUrl of traceUrls) {
    ...
    if (
      ...
      !_fs.default.existsSync(traceFile) &&
      !_fs.default.existsSync(traceFile + ".trace")
    )
...
async function startTraceViewerServer(options) {
  ...
  const transport =
    ...
    (options !== null && options !== void 0 && options.isServer ? new StdinServer() : undefined);
...
async function openTraceViewerApp(url, browserName, options) {
  ...
  const { context, page } = await (0, _launchApp.launchApp)(
    traceViewerPlaywright[traceViewerBrowser],
    {
...
class StdinServer {
  ...
  _loadTrace(traceUrl) {
    ...
    (_this$sendEvent = this.sendEvent) === null ||
      _this$sendEvent === void 0 ||
      _this$sendEvent.call(this, "loadTraceRequested", {
        traceUrl,
      });
  }
  _pollLoadTrace(url) {
