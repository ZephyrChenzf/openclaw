...
class WebSocketTransport {
  ...
  static async _connect(progress, url, headers, redirect, debugLogHeader) {
    ...
    const transport = new WebSocketTransport(
      ...
      logUrl,
      headers,
      redirect.follow && redirect.hadRedirects,
      debugLogHeader,
    );
    let success = false;
    progress === null ||
      ...
      transport._ws.on("unexpected-response", (request, response) => {
        ...
        for (let i = 0; i < response.rawHeaders.length; i += 2) {
          if (debugLogHeader && response.rawHeaders[i] === debugLogHeader)
            progress === null || progress === void 0 || progress.log(response.rawHeaders[i + 1]);
        }
        const chunks = [];
  ...
  constructor(progress, url, logUrl, headers, followRedirects, debugLogHeader) {
    ...
    this._ws.addEventListener("message", (event) => {
      messageWrap(() => {
        ...
        try {
          ...
          var _this$_progress;
          (_this$_progress = this._progress) === null ||
            _this$_progress === void 0 ||
            _this$_progress.log(
              `<closing ws> Closing websocket due to malformed JSON. eventData=${eventData} e=${e === null || e === void 0 ? void 0 : e.message}`,
            );
          this._ws.close();
          return;
        }
        try {
          ...
          (_this$_progress2 = this._progress) === null ||
            _this$_progress2 === void 0 ||
            _this$_progress2.log(
              `<closing ws> Closing websocket due to failed onmessage callback. eventData=${eventData} e=${e === null || e === void 0 ? void 0 : e.message}`,
            );
          this._ws.close();
        }
      });
    ...
    this._ws.addEventListener("close", (event) => {
      ...
      (_this$_progress3 = this._progress) === null ||
        _this$_progress3 === void 0 ||
        _this$_progress3.log(
          `<ws disconnected> ${logUrl} code=${event.code} reason=${event.reason}`,
        );
    ...
    this._ws.addEventListener("error", (error) => {
      ...
      return (_this$_progress4 = this._progress) === null || _this$_progress4 === void 0
        ? void 0
        : _this$_progress4.log(`<ws error> ${logUrl} ${error.type} ${error.message}`);
    });
  }
  send(message) {
  ...
  close() {
    ...
    (_this$_progress5 = this._progress) === null ||
      _this$_progress5 === void 0 ||
      _this$_progress5.log(`<ws disconnecting> ${this._logUrl}`);
    this._ws.close();
  }
  async closeAndWait() {
