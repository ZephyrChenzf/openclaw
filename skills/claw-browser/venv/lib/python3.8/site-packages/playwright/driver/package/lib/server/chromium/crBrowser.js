...
class CRBrowser extends _browser.Browser {
  ...
  async startTracing(page, options = {}) {
    ...
    const { screenshots = false, categories = defaultCategories } = options;
  ...
  async stopTracing() {
    ...
    const tracingPath = _path.default.join(
      this.options.artifactsDir,
      (0, _utils.createGuid)() + ".crtrace",
    );
...
class CRBrowserContext extends _browserContext.BrowserContext {
  ...
  async _initialize() {
    (0, _utils.assert)(
      !Array.from(this._browser._crPages.values()).some((page) => page._browserContext === this),
    );
  ...
  pages() {
    return this._crPages()
      .map((crPage) => crPage._initializedPage)
      .filter(Boolean);
  }
  async newPageDelegate() {
    (0, _browserContext.assertBrowserContextIsNotOwned)(this);
    const oldKeys = this._browser.isClank() ? new Set(this._browser._crPages.keys()) : undefined;
    let { targetId } = await this._browser._session.send("Target.createTarget", {
      url: "about:blank",
      browserContextId: this._browserContextId,
    });
    if (oldKeys) {
  ...
  async doClose(reason) {
    ...
    for (const crPage of this._crPages()) {
      const dialogs = [...crPage._page._frameManager._openedDialogs].filter(
        (dialog) => dialog.type() === "beforeunload",
      );
  ...
  async stopVideoRecording() {
    await Promise.all(
      this._crPages().map((crPage) => crPage._mainFrameSession._stopVideoRecording()),
    );
  }
  onClosePersistent() {
