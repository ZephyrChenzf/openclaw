...
class BrowserContext extends _instrumentation.SdkObject {
  ...
  async _ensureVideosPath() {
    if (this._options.recordVideo)
      await (0, _fileUtils.mkdirIfNeeded)(
  ...
  async resetForReuse(metadata, params) {
    ...
    (_page = page) === null ||
      _page === void 0 ||
      _page._frameManager.setCloseAllOpeningDialogs(true);
    await ((_page2 = page) === null || _page2 === void 0
      ? void 0
      : _page2._frameManager.closeOpenDialogs());
    // Navigate to about:blank first to ensure no page scripts are running after this point.
    await ((_page3 = page) === null || _page3 === void 0
      ? void 0
      : _page3.mainFrame().goto(metadata, "about:blank", {
          timeout: 0,
        }));
    (_page4 = page) === null ||
      _page4 === void 0 ||
      _page4._frameManager.setCloseAllOpeningDialogs(false);
  ...
  _didCloseInternal() {
    ...
    (_this$_clientCertific = this._clientCertificatesProxy) === null ||
      _this$_clientCertific === void 0 ||
      _this$_clientCertific.close().catch(() => {});
    this.tracing.abort();
  ...
  async clearCookies(options) {
    ...
    const cookiesToReadd = currentCookies.filter((cookie) => {
      return (
        !matches(cookie, "name", options.name) ||
  ...
  async exposeBinding(name, needsHandle, playwrightBinding) {
    ...
    const frames = this.pages()
      .map((page) => page.frames())
      .flat();
    await Promise.all(
      frames.map((frame) => frame.evaluateExpression(binding.initScript.source).catch((e) => {})),
    );
  }
  ...
  async _loadDefaultContext(progress) {
    ...
    if (
      (this._options.isMobile && browserName === "chromium") ||
      (this._options.locale && browserName === "webkit")
    ) {
      // Workaround for:
  ...
  async _deleteAllTempDirs() {
    await Promise.all(
      this._tempDirs.map(async (dir) => await _fs.default.promises.unlink(dir).catch((e) => {})),
    );
  }
  ...
  async _resetStorage() {
    ...
    const newOrigins = new Map(
      ((_this$_options$storag = this._options.storageState) === null ||
      _this$_options$storag === void 0 ||
      (_this$_options$storag = _this$_options$storag.origins) === null ||
      _this$_options$storag === void 0
        ? void 0
        : _this$_options$storag.map((p) => [p.origin, p])) || [],
    );
  ...
  async _resetCookies() {
    ...
    await this.doClearCookies();
    if (
      (_this$_options$storag2 = this._options.storageState) !== null &&
      _this$_options$storag2 !== void 0 &&
      _this$_options$storag2.cookies
    )
      await this.addCookies(
        (_this$_options$storag3 = this._options.storageState) === null ||
          _this$_options$storag3 === void 0
          ? void 0
          : _this$_options$storag3.cookies,
      );
  }
  ...
  async safeNonStallingEvaluateInAllFrames(expression, world, options = {}) {
    await Promise.all(
      this.pages().map((page) =>
        page.safeNonStallingEvaluateInAllFrames(expression, world, options),
      ),
    );
  }
