...
class FFPage {
  constructor(session, browserContext, opener) {
    ...
    this._eventListeners = [
      ...
      _eventsHelper.eventsHelper.addEventListener(this._session, "Page.linkClicked", (event) =>
        this._onLinkClicked(event.phase),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        this._session,
        "Page.uncaughtError",
    ...
    this.addInitScript(new _page.InitScript("", true), UTILITY_WORLD_NAME).catch((e) =>
      this._markAsError(e),
    );
  }
  ...
  async _onWorkerCreated(event) {
    ...
    workerSession.once("Runtime.executionContextCreated", (event) => {
      worker._createExecutionContext(
        new _ffExecutionContext.FFExecutionContext(workerSession, event.executionContextId),
      );
    });
  ...
  async updateEmulateMedia() {
    ...
    const colorScheme =
      emulatedMedia.colorScheme === "no-override" ? undefined : emulatedMedia.colorScheme;
    const reducedMotion =
      emulatedMedia.reducedMotion === "no-override" ? undefined : emulatedMedia.reducedMotion;
    const forcedColors =
      emulatedMedia.forcedColors === "no-override" ? undefined : emulatedMedia.forcedColors;
