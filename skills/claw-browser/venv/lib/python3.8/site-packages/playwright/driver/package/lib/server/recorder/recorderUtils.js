...
function metadataToCallLog(metadata, status) {
  ...
  const params = {
    url:
      (_metadata$params = metadata.params) === null || _metadata$params === void 0
        ? void 0
        : _metadata$params.url,
    selector:
      (_metadata$params2 = metadata.params) === null || _metadata$params2 === void 0
        ? void 0
  ...
  const callLog = {
    ...
    status,
    error:
      (_metadata$error = metadata.error) === null ||
      _metadata$error === void 0 ||
      (_metadata$error = _metadata$error.error) === null ||
      _metadata$error === void 0
        ? void 0
        : _metadata$error.message,
    params,
    duration,
  };
  return callLog;
...
function mainFrameForAction(pageAliases, actionInContext) {
  ...
  const page =
    (_find = [...pageAliases.entries()].find(([, alias]) => pageAlias === alias)) === null ||
    _find === void 0
      ? void 0
      : _find[0];
  if (!page)
    throw new Error(
      `Internal error: page ${pageAlias} not found in [${[...pageAliases.values()]}]`,
    );
...
async function frameForAction(pageAliases, actionInContext, action) {
  ...
  const pageAlias = actionInContext.frame.pageAlias;
  const page =
    (_find2 = [...pageAliases.entries()].find(([, alias]) => pageAlias === alias)) === null ||
    _find2 === void 0
      ? void 0
      : _find2[0];
  if (!page) throw new Error("Internal error: page not found");
  const fullSelector = (0, _recorderUtils.buildFullSelector)(
    actionInContext.frame.framePath,
    action.selector,
  );
...
function callMetadataForAction(pageAliases, actionInContext) {
  ...
  const { method, params } = (0, _recorderUtils.traceParamsForAction)(actionInContext);
  const callMetadata = {
...
function collapseActions(actions) {
  ...
  for (const action of actions) {
    ...
    const isSameAction =
      lastAction &&
      lastAction.action.name === action.action.name &&
      lastAction.frame.pageAlias === action.frame.pageAlias &&
      lastAction.frame.framePath.join("|") === action.frame.framePath.join("|");
    const isSameSelector =
      lastAction &&
      "selector" in lastAction.action &&
      "selector" in action.action &&
      action.action.selector === lastAction.action.selector;
    const shouldMerge =
      isSameAction &&
      (action.action.name === "navigate" || (action.action.name === "fill" && isSameSelector));
    if (!shouldMerge) {
      result.push(action);
      continue;
    }
