...
class DebugController extends _instrumentation.SdkObject {
  ...
  async setAutoCloseEnabled(enabled) {
    ...
    const heartBeat = () => {
      ...
      else this._autoCloseTimer = setTimeout(heartBeat, 5000);
    };
  ...
  async _allRecorders() {
    ...
    const result = await Promise.all(
      [...contexts].map((c) =>
        _recorder.Recorder.showInspector(
          c,
          {
...
class InspectingRecorderApp extends _recorderApp.EmptyRecorderApp {
  ...
  async setSources(sources) {
    ...
    const { text, header, footer, actions } = source || {
      text: "",
    };
