...
class BrowserContextDispatcher extends _dispatcher.Dispatcher {
  constructor(parentScope, context) {
    ...
    const requestContext = _networkDispatchers.APIRequestContextDispatcher.from(
      parentScope,
      ...
      if (this._shouldDispatchEvent(page, "console")) {
        ...
        this._dispatchEvent("console", {
          ...
          args: message
            .args()
            .map((a) =>
              _elementHandlerDispatcher.ElementHandleDispatcher.fromJSHandle(pageDispatcher, a),
            ),
    ...
    if (context._browser.options.name === "chromium") {
      ...
      this.addObjectListener(_crBrowser.CRBrowserContext.CREvents.BackgroundPage, (page) =>
        this._dispatchEvent("backgroundPage", {
          page: _pageDispatcher.PageDispatcher.from(this, page),
        }),
      );
      ...
      this.addObjectListener(_crBrowser.CRBrowserContext.CREvents.ServiceWorker, (serviceWorker) =>
        this._dispatchEvent("serviceWorker", {
          worker: new _pageDispatcher.WorkerDispatcher(this, serviceWorker),
        }),
      );
    ...
    this.addObjectListener(_browserContext.BrowserContext.Events.Request, (request) => {
      ...
      const redirectFromDispatcher =
        request.redirectedFrom() && (0, _dispatcher.existingDispatcher)(request.redirectedFrom());
      if (
        !redirectFromDispatcher &&
        !this._shouldDispatchNetworkEvent(request, "request") &&
      ...
      this._dispatchEvent("request", {
        ...
        page: _pageDispatcher.PageDispatcher.fromNullable(
          this,
          (_request$frame = request.frame()) === null || _request$frame === void 0
            ? void 0
            : _request$frame._page.initializedOrUndefined(),
        ),
      });
    });
    this.addObjectListener(_browserContext.BrowserContext.Events.Response, (response) => {
      ...
      this._dispatchEvent("response", {
        ...
        page: _pageDispatcher.PageDispatcher.fromNullable(
          this,
          (_response$frame = response.frame()) === null || _response$frame === void 0
            ? void 0
            : _response$frame._page.initializedOrUndefined(),
        ),
      });
    });
    this.addObjectListener(_browserContext.BrowserContext.Events.RequestFailed, (request) => {
      ...
      this._dispatchEvent("requestFailed", {
        ...
        page: _pageDispatcher.PageDispatcher.fromNullable(
          this,
          (_request$frame2 = request.frame()) === null || _request$frame2 === void 0
            ? void 0
            : _request$frame2._page.initializedOrUndefined(),
        ),
      });
    });
    this.addObjectListener(
      ...
      ({ request, response }) => {
        ...
        this._dispatchEvent("requestFinished", {
          ...
          page: _pageDispatcher.PageDispatcher.fromNullable(
            this,
            (_request$frame3 = request.frame()) === null || _request$frame3 === void 0
              ? void 0
              : _request$frame3._page.initializedOrUndefined(),
          ),
        });
      },
    );
  ...
  _shouldDispatchNetworkEvent(request, event) {
    var _request$frame4;
    return this._shouldDispatchEvent(
      (_request$frame4 = request.frame()) === null ||
        _request$frame4 === void 0 ||
        (_request$frame4 = _request$frame4._page) === null ||
        _request$frame4 === void 0
        ? void 0
        : _request$frame4.initializedOrUndefined(),
      event,
    );
  }
  _shouldDispatchEvent(page, event) {
    ...
    if (
      pageDispatcher !== null &&
      pageDispatcher !== void 0 &&
      pageDispatcher._subscriptions.has(event)
    )
      return true;
    return false;
  }
  async createTempFiles(params) {
    ...
    const tempDirWithRootName = params.rootDirName
      ? path.join(tmpDir, path.basename(params.rootDirName))
      : tmpDir;
    ...
    return {
      rootDir: params.rootDirName
        ? new _writableStreamDispatcher.WritableStreamDispatcher(this, tempDirWithRootName)
        : undefined,
      writableStreams: await Promise.all(
        params.items.map(async (item) => {
          ...
          return new _writableStreamDispatcher.WritableStreamDispatcher(
            this,
            file,
            item.lastModifiedMs,
          );
  ...
  async clearCookies(params) {
    const nameRe =
      params.nameRegexSource !== undefined && params.nameRegexFlags !== undefined
        ? new RegExp(params.nameRegexSource, params.nameRegexFlags)
        : undefined;
    const domainRe =
      params.domainRegexSource !== undefined && params.domainRegexFlags !== undefined
        ? new RegExp(params.domainRegexSource, params.domainRegexFlags)
        : undefined;
    const pathRe =
      params.pathRegexSource !== undefined && params.pathRegexFlags !== undefined
        ? new RegExp(params.pathRegexSource, params.pathRegexFlags)
        : undefined;
  ...
  async setNetworkInterceptionPatterns(params) {
    ...
    const urlMatchers = params.patterns.map((pattern) =>
      pattern.regexSource ? new RegExp(pattern.regexSource, pattern.regexFlags) : pattern.glob,
    );
    await this._context.setRequestInterceptor((route, request) => {
      const matchesSome = urlMatchers.some((urlMatch) =>
        (0, _utils.urlMatches)(this._context._options.baseURL, request.url(), urlMatch),
      );
      if (!matchesSome) return false;
      this._dispatchEvent("route", {
        route: _networkDispatchers.RouteDispatcher.from(
          _networkDispatchers.RequestDispatcher.from(this, request),
          route,
        ),
      });
      return true;
  ...
  async enableRecorder(params) {
    if (params.codegenMode === "trace-events") {
      ...
      await _recorder.Recorder.show(
        "trace-events",
        this._context,
        _recorderInTraceViewer.RecorderInTraceViewer.factory(this._context),
        params,
      );
    } else {
      await _recorder.Recorder.show(
        "actions",
        this._context,
        _recorderApp.RecorderApp.factory(this._context),
        params,
      );
    }
  }
  ...
  async newCDPSession(params) {
    ...
    return {
      session: new _cdpSessionDispatcher.CDPSessionDispatcher(
        this,
        await crBrowserContext.newCDPSession((params.page ? params.page : params.frame)._object),
      ),
    };
  }
  async harStart(params) {
    const harId = await this._context._harStart(
      params.page ? params.page._object : null,
      params.options,
    );
    return {
      harId,
  ...
  async clockFastForward(params, metadata) {
    ...
    await this._context.clock.fastForward(
      (_ref =
        (_params$ticksString = params.ticksString) !== null && _params$ticksString !== void 0
          ? _params$ticksString
          : params.ticksNumber) !== null && _ref !== void 0
        ? _ref
        : 0,
    );
  }
  async clockInstall(params, metadata) {
    ...
    await this._context.clock.install(
      (_ref2 =
        (_params$timeString = params.timeString) !== null && _params$timeString !== void 0
          ? _params$timeString
          : params.timeNumber) !== null && _ref2 !== void 0
        ? _ref2
        : undefined,
    );
  }
  async clockPauseAt(params, metadata) {
    ...
    await this._context.clock.pauseAt(
      (_ref3 =
        (_params$timeString2 = params.timeString) !== null && _params$timeString2 !== void 0
          ? _params$timeString2
          : params.timeNumber) !== null && _ref3 !== void 0
        ? _ref3
        : 0,
    );
  }
  ...
  async clockRunFor(params, metadata) {
    ...
    await this._context.clock.runFor(
      (_ref4 =
        (_params$ticksString2 = params.ticksString) !== null && _params$ticksString2 !== void 0
          ? _params$ticksString2
          : params.ticksNumber) !== null && _ref4 !== void 0
        ? _ref4
        : 0,
    );
  }
  async clockSetFixedTime(params, metadata) {
    ...
    await this._context.clock.setFixedTime(
      (_ref5 =
        (_params$timeString3 = params.timeString) !== null && _params$timeString3 !== void 0
          ? _params$timeString3
          : params.timeNumber) !== null && _ref5 !== void 0
        ? _ref5
        : 0,
    );
  }
  async clockSetSystemTime(params, metadata) {
    ...
    await this._context.clock.setSystemTime(
      (_ref6 =
        (_params$timeString4 = params.timeString) !== null && _params$timeString4 !== void 0
          ? _params$timeString4
          : params.timeNumber) !== null && _ref6 !== void 0
        ? _ref6
        : 0,
    );
  }
