...
class Chromium extends _browserType.BrowserType {
  ...
  async _connectOverCDPInternal(progress, endpointURL, options, onClose) {
    ...
    if (!headersMap)
      ...
      headersMap &&
      !Object.keys(headersMap).some((key) => key.toLowerCase() === "user-agent")
    )
  ...
  _createDevTools() {
    ...
    return directory
      ? new _crDevTools.CRDevTools(_path.default.join(directory, "devtools-preferences.json"))
      : undefined;
  }
  ...
  doRewriteStartupLog(error) {
    ...
    if (
      !error.logs.includes("crbug.com/357670") &&
      !error.logs.includes("No usable sandbox!") &&
      !error.logs.includes("crbug.com/638180")
    )
      return error;
    error.logs = [
  ...
  async _launchWithSeleniumHub(progress, hubUrl, options) {
    ...
    try {
      ...
      if (capabilities["se:cdp"]) {
        ...
        progress.log(
          `<selenium> retrieved endpoint ${endpointURL.toString()} for sessionId=${sessionId}`,
        );
      } else {
        ...
        const endpointURLString = addProtocol(debuggerAddress || chromeOptionsURL).replace(
          "localhost",
          "127.0.0.1",
        );
        progress.log(
          `<selenium> retrieved endpoint ${endpointURLString} for sessionId=${sessionId}`,
        );
        endpointURL = new URL(endpointURLString);
        if (endpointURL.hostname === "localhost" || endpointURL.hostname === "127.0.0.1") {
          const sessionInfoUrl =
            new URL(hubUrl).origin + "/grid/api/testsession?session=" + sessionId;
          try {
            ...
            progress.log(
              `<selenium> resolved endpoint ip ${endpointURL.toString()} for sessionId=${sessionId}`,
            );
          } catch (e) {
            progress.log(
              `<selenium> unable to resolve endpoint ip for sessionId=${sessionId}, running in standalone?`,
            );
          }
        }
      }
  ...
  _innerDefaultArgs(options) {
    ...
    if (proxy) {
      ...
      if (
        ...
        !proxyBypassRules.includes("<-loopback>")
      )
  ...
  readyState(options) {
    ...
    if (
      ...
      ((_options$args = options.args) !== null &&
        _options$args !== void 0 &&
