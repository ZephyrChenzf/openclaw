...
class BidiExecutionContext {
  ...
  async rawEvaluateHandle(expression) {
    ...
    if (response.type === "success") {
      ...
      throw new js.JavaScriptErrorInEvaluate(
        "Cannot get handle: " + JSON.stringify(response.result),
      );
    }
    if (response.type === "exception")
      throw new js.JavaScriptErrorInEvaluate(
    ...
    if (response.type === "success") {
      if (returnByValue)
        return (0, _utilityScriptSerializers.parseEvaluationResultValue)(
          _bidiDeserializer.BidiDeserializer.deserialize(response.result),
        );
  ...
  createHandle(context, jsRemoteObject) {
    ...
    return new js.JSHandle(
      ...
      jsRemoteObject.objectId,
      remoteObjectValue(remoteObject),
    );
  }
