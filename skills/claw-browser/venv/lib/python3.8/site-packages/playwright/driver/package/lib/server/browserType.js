...
const kNoXServerRunningError = (exports.kNoXServerRunningError =
...
class BrowserType extends _instrumentation.SdkObject {
  ...
  executablePath() {
    return (
      _registry.registry
        ...
        .executablePath(this.attribution.playwright.options.sdkLanguage) || ""
    );
  }
  name() {
    return this._name;
  ...
  async launchPersistentContext(metadata, userDataDir, options) {
    ...
    const browser = await controller.run(async (progress) => {
      ...
      if (
        (_options$clientCertif = options.clientCertificates) !== null &&
        _options$clientCertif !== void 0 &&
        _options$clientCertif.length
      ) {
        var _clientCertificatesPr;
        clientCertificatesProxy = new _socksClientCertificatesInterceptor.ClientCertificatesProxy(
          options,
        );
        launchOptions.proxyOverride = await ((_clientCertificatesPr = clientCertificatesProxy) ===
          null || _clientCertificatesPr === void 0
          ? void 0
          : _clientCertificatesPr.listen());
        options = {
          ...options,
        };
      ...
      progress.cleanupWhenAborted(() => {
        ...
        return (_clientCertificatesPr2 = clientCertificatesProxy) === null ||
          _clientCertificatesPr2 === void 0
          ? void 0
          : _clientCertificatesPr2.close();
      });
      const browser = await this._innerLaunchWithRetries(
        progress,
        launchOptions,
        options,
  ...
  async _innerLaunch(progress, options, persistent, protocolLogger, maybeUserDataDir) {
    ...
    const { browserProcess, userDataDir, artifactsDir, transport } = await this._launchProcess(
      progress,
      options,
      !!persistent,
  ...
  async _launchProcess(progress, options, isPersistent, browserLogsCollector, userDataDir) {
    ...
    const artifactsDir = await _fs.default.promises.mkdtemp(
      _path.default.join(os.tmpdir(), "playwright-artifacts-"),
    );
    ...
    if (executablePath) {
      ...
      executable = registryExecutable.executablePathOrDie(
        this.attribution.playwright.options.sdkLanguage,
      );
      await _registry.registry.validateHostRequirementsForExecutablesIfNeeded(
        [registryExecutable],
        this.attribution.playwright.options.sdkLanguage,
      );
    }
    ...
    const { launchedProcess, gracefullyClose, kill } = await (0, _processLauncher.launchProcess)({
      command: executable,
    ...
    browserProcess = {
      ...
      close: () =>
        closeOrKill(options.__testHookBrowserCloseTimeout || _timeoutSettings.DEFAULT_TIMEOUT),
      kill,
    };
    progress.cleanupWhenAborted(() => closeOrKill(progress.timeUntilDeadline()));
    const wsEndpoint =
      (_await$readyState$wai = await (readyState === null || readyState === void 0
        ? void 0
        : readyState.waitUntilReady())) === null || _await$readyState$wai === void 0
        ? void 0
        : _await$readyState$wai.wsEndpoint;
  ...
  _createUserDataDirArgMisuseError(userDataDirArg) {
    switch (this.attribution.playwright.options.sdkLanguage) {
      case "java":
        return new Error(
          `Pass userDataDir parameter to 'BrowserType.launchPersistentContext(userDataDir, options)' instead of specifying '${userDataDirArg}' argument`,
        );
      case "python":
        return new Error(
          `Pass user_data_dir parameter to 'browser_type.launch_persistent_context(user_data_dir, **kwargs)' instead of specifying '${userDataDirArg}' argument`,
        );
      case "csharp":
        return new Error(
          `Pass userDataDir parameter to 'BrowserType.LaunchPersistentContextAsync(userDataDir, options)' instead of specifying '${userDataDirArg}' argument`,
        );
      default:
        return new Error(
          `Pass userDataDir parameter to 'browserType.launchPersistentContext(userDataDir, options)' instead of specifying '${userDataDirArg}' argument`,
