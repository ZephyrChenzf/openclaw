...
class Tracing extends _instrumentation.SdkObject {
  constructor(context, tracesDir) {
    ...
    const testIdAttributeName =
      "selectors" in context ? context.selectors().testIdAttributeName() : undefined;
  ...
  async resetForReuse() {
    ...
    await this.stop();
    (_this$_snapshotter = this._snapshotter) === null ||
      _this$_snapshotter === void 0 ||
      _this$_snapshotter.resetForReuse();
  }
  async start(options) {
    ...
    this._contextCreatedEvent.sdkLanguage =
      this._context.attribution.playwright.options.sdkLanguage;
  ...
  async startChunk(options = {}) {
    ...
    this._eventListeners.push(
      _utils.eventsHelper.addEventListener(
        this._context,
        _browserContext.BrowserContext.Events.Console,
        this._onConsoleMessage.bind(this),
      ),
      _utils.eventsHelper.addEventListener(
        this._context,
        _browserContext.BrowserContext.Events.PageError,
        this._onPageError.bind(this),
      ),
    );
    if (this._state.options.screenshots) this._startScreencast();
    if (this._state.options.snapshots)
      await ((_this$_snapshotter2 = this._snapshotter) === null || _this$_snapshotter2 === void 0
        ? void 0
        : _this$_snapshotter2.start());
    return {
  ...
  _createTracesDirIfNeeded() {
    ...
    this._tracesTmpDir = _fs.default.mkdtempSync(
      _path.default.join(_os.default.tmpdir(), "playwright-tracing-"),
    );
    return this._tracesTmpDir;
  }
  abort() {
    var _this$_snapshotter3;
    (_this$_snapshotter3 = this._snapshotter) === null ||
      _this$_snapshotter3 === void 0 ||
      _this$_snapshotter3.dispose();
    this._harTracer.stop();
  }
  async flush() {
    this.abort();
  ...
  async stopChunk(params) {
    ...
    if (this._state.options.snapshots)
      await ((_this$_snapshotter4 = this._snapshotter) === null || _this$_snapshotter4 === void 0
        ? void 0
        : _this$_snapshotter4.stop());
    this.flushHarEntries();
    ...
    const newNetworkFile = _path.default.join(
      this._state.tracesDir,
      this._state.traceName + `-pwnetcopy-${this._state.chunkOrdinal}.network`,
    );
    const entries = [];
    entries.push({
      name: "trace.trace",
    ...
    if (error) {
      ...
      if (
        this._context instanceof _browserContext.BrowserContext &&
        !this._context._browser.isConnected()
      )
        return {};
      throw error;
    }
  ...
  async _captureSnapshot(snapshotName, sdkObject, metadata, element) {
    ...
    await this._snapshotter
      .captureSnapshot(sdkObject.attribution.page, metadata.id, snapshotName, element)
      .catch(() => {});
  }
  onBeforeCall(sdkObject, metadata) {
    ...
    (_sdkObject$attributio = sdkObject.attribution.page) === null ||
      ...
      _sdkObject$attributio.temporarilyDisableTracingScreencastThrottling();
    event.beforeSnapshot = `before@${metadata.id}`;
    (_this$_state = this._state) === null ||
      _this$_state === void 0 ||
      _this$_state.callIds.add(metadata.id);
    this._appendTraceEvent(event);
  ...
  onBeforeInputAction(sdkObject, metadata, element) {
    ...
    if (
      !(
        (_this$_state2 = this._state) !== null &&
        _this$_state2 !== void 0 &&
        _this$_state2.callIds.has(metadata.id)
      )
    )
    ...
    (_sdkObject$attributio2 = sdkObject.attribution.page) === null ||
      ...
      _sdkObject$attributio2.temporarilyDisableTracingScreencastThrottling();
  ...
  async onAfterCall(sdkObject, metadata) {
    ...
    if (
      !(
        (_this$_state3 = this._state) !== null &&
        _this$_state3 !== void 0 &&
        _this$_state3.callIds.has(metadata.id)
      )
    )
      return;
    (_this$_state4 = this._state) === null ||
      _this$_state4 === void 0 ||
      _this$_state4.callIds.delete(metadata.id);
    const event = createAfterActionTraceEvent(metadata);
    if (!event) return;
    (_sdkObject$attributio3 = sdkObject.attribution.page) === null ||
      _sdkObject$attributio3 === void 0 ||
      _sdkObject$attributio3.temporarilyDisableTracingScreencastThrottling();
  ...
  _onConsoleMessage(message) {
    ...
    const event = {
      ...
      pageId:
        (_message$page = message.page()) === null || _message$page === void 0
          ? void 0
          : _message$page.guid,
    };
    this._appendTraceEvent(event);
  ...
  onPageOpen(page) {
    ...
    const event = {
      ...
      params: {
        pageId: page.guid,
        openerPageId:
          (_page$opener = page.opener()) === null || _page$opener === void 0
            ? void 0
            : _page$opener.guid,
      },
    };
  ...
  _startScreencastInPage(page) {
    ...
    this._screencastListeners.push(
      _utils.eventsHelper.addEventListener(page, _page.Page.Events.ScreencastFrame, (params) => {
  ...
  _appendTraceEvent(event) {
    ...
    const flush =
      this._state.options.live ||
      (event.type !== "event" && event.type !== "console" && event.type !== "log");
...
function createAfterActionTraceEvent(metadata) {
  ...
  return {
    ...
    error:
      (_metadata$error = metadata.error) === null || _metadata$error === void 0
        ? void 0
