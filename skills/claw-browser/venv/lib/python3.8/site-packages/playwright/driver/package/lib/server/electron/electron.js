...
class ElectronApplication extends _instrumentation.SdkObject {
  constructor(parent, browser, nodeConnection, process) {
    ...
    this._nodeSession.on("Runtime.executionContextCreated", async (event) => {
      ...
      const crExecutionContext = new _crExecutionContext.CRExecutionContext(
        this._nodeSession,
        event.context,
      );
      ...
      this._nodeElectronHandlePromise.resolve(
        new js.JSHandle(
  ...
  async launch(options) {
    ...
    return controller.run(async (progress) => {
      ...
      const { launchedProcess, gracefullyClose, kill } = await (0, _processLauncher.launchProcess)({
        ...
        onExit: () => {
          var _app;
          return (_app = app) === null || _app === void 0
            ? void 0
            : _app.emit(ElectronApplication.Events.Close);
        },
      });
      ...
      const nodeMatchPromise = waitForLine(
        progress,
        launchedProcess,
        /^Debugger listening on (ws:\/\/.*)$/,
      );
      const chromeMatchPromise = waitForLine(
        progress,
        launchedProcess,
        /^DevTools listening on (ws:\/\/.*)$/,
      );
      const debuggerDisconnectPromise = waitForLine(
        progress,
        launchedProcess,
...
function waitForLine(progress, process, regex) {
  return new Promise((resolve, reject) => {
    ...
    const listeners = [
      ...
      _eventsHelper.eventsHelper.addEventListener(process, "exit", reject.bind(null, failError)),
