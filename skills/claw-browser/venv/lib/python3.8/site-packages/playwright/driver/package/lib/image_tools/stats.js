...
class FastStats {
  ...
  varianceC1(x1, y1, x2, y2) {
    const N = (y2 - y1 + 1) * (x2 - x1 + 1);
    return (
      (this._sum(this._partialSumSq1, x1, y1, x2, y2) -
        this._sum(this._partialSumC1, x1, y1, x2, y2) ** 2 / N) /
      N
    );
  }
  varianceC2(x1, y1, x2, y2) {
    const N = (y2 - y1 + 1) * (x2 - x1 + 1);
    return (
      (this._sum(this._partialSumSq2, x1, y1, x2, y2) -
        this._sum(this._partialSumC2, x1, y1, x2, y2) ** 2 / N) /
      N
    );
  }
  covariance(x1, y1, x2, y2) {
    const N = (y2 - y1 + 1) * (x2 - x1 + 1);
    return (
      (this._sum(this._partialSumMult, x1, y1, x2, y2) -
        (this._sum(this._partialSumC1, x1, y1, x2, y2) *
          this._sum(this._partialSumC2, x1, y1, x2, y2)) /
          N) /
      N
    );
  }
