...
class Page extends _channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on("fileChooser", ({ element, isMultiple }) =>
      this.emit(
        ...
        new _fileChooser.FileChooser(this, _elementHandle.ElementHandle.from(element), isMultiple),
      ),
    );
    ...
    this._channel.on("route", ({ route }) => this._onRoute(_network.Route.from(route)));
    ...
    this._channel.on("worker", ({ worker }) => this._onWorker(_worker.Worker.from(worker)));
    this.coverage = new _coverage.Coverage(this._channel);
    this.once(_events.Events.Page.Close, () =>
      this._closedOrCrashedScope.close(this._closeErrorWithReason()),
    );
    this.once(_events.Events.Page.Crash, () =>
      this._closedOrCrashedScope.close(new _errors.TargetClosedError()),
    );
    this._setEventToSubscriptionMapping(
      new Map([
        [_events.Events.Page.Console, "console"],
        [_events.Events.Page.Dialog, "dialog"],
        [_events.Events.Page.Request, "request"],
        [_events.Events.Page.Response, "response"],
        [_events.Events.Page.RequestFinished, "requestFinished"],
        [_events.Events.Page.RequestFailed, "requestFailed"],
        [_events.Events.Page.FileChooser, "fileChooser"],
      ]),
    );
  }
  ...
  _closeErrorWithReason() {
    return new _errors.TargetClosedError(
      this._closeReason || this._browserContext._effectiveCloseReason(),
    );
  }
  ...
  async route(url, handler, options = {}) {
    this._routes.unshift(
      new _network.RouteHandler(this._browserContext._options.baseURL, url, handler, options.times),
    );
  ...
  async routeWebSocket(url, handler) {
    this._webSocketRoutes.unshift(
      new _network.WebSocketRouteHandler(this._browserContext._options.baseURL, url, handler),
    );
  ...
  async _updateWebSocketInterceptionPatterns() {
    const patterns = _network.WebSocketRouteHandler.prepareInterceptionPatterns(
      this._webSocketRoutes,
    );
  ...
  async _expectScreenshot(options) {
    const mask =
      options !== null && options !== void 0 && options.mask
  ...
  async pause() {
    ...
    (_this$_instrumentatio = this._instrumentation) === null ||
      _this$_instrumentatio === void 0 ||
      _this$_instrumentatio.onWillPause();
