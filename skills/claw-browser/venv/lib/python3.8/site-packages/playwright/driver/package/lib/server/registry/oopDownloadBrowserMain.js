...
function log(message) {
  ...
  (_process$send = (_process = process).send) === null ||
...
function progress(done, total) {
  ...
  (_process$send2 = (_process2 = process).send) === null ||
    _process$send2 === void 0 ||
    ...
    (response) => {
      ...
      if (response.statusCode !== 200) {
        ...
        const handleError = () => {
          const error = new Error(
            `Download failed: server returned code ${response.statusCode} body '${content}'. URL: ${options.url}`,
          );
      ...
      file.on("finish", () => {
        if (downloadedBytes !== totalBytes) {
          ...
          promise.reject(
            new Error(
              `Download failed: size mismatch, file size: ${downloadedBytes}, expected size: ${totalBytes} URL: ${options.url}`,
            ),
          );
        } else {
      ...
      response.on("error", (error) => {
        ...
        if ((error === null || error === void 0 ? void 0 : error.code) === "ECONNRESET") {
          ...
          promise.reject(
            new Error(`Download failed: server closed connection. URL: ${options.url}`),
          );
        } else {
          var _error$message;
          ...
          promise.reject(
            new Error(
              `Download failed: ${(_error$message = error === null || error === void 0 ? void 0 : error.message) !== null && _error$message !== void 0 ? _error$message : error}. URL: ${options.url}`,
            ),
          );
        }
      });
...
async function main(options) {
  ...
  await _fs.default.promises.writeFile(
    browserDirectoryToMarkerFilePath(options.browserDirectory),
    "",
  );
}
