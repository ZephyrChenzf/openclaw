...
class JavaScriptLanguageGenerator {
  ...
  generateAction(actionInContext) {
    ...
    const locators = actionInContext.frame.framePath.map(
      (selector) => `.${this._asLocator(selector)}.contentFrame()`,
    );
    ...
    formatter.add(
      wrapWithStep(actionInContext.description, this._generateActionCall(subject, actionInContext)),
    );
    if (signals.popup)
      formatter.add(
        `const ${signals.popup.popupAlias} = await ${signals.popup.popupAlias}Promise;`,
      );
    if (signals.download)
      formatter.add(
        `const download${signals.download.downloadAlias} = await download${signals.download.downloadAlias}Promise;`,
      );
    return formatter.format();
  }
