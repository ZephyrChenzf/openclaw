...
class BidiPage {
  constructor(browserContext, bidiSession, opener) {
    ...
    this._networkManager = new _bidiNetworkManager.BidiNetworkManager(
      this._session,
      this._page,
      this._onNavigationResponseStarted.bind(this),
    );
    ...
    this._sessionListeners = [
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "script.realmCreated",
        this._onRealmCreated.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "script.message",
        this._onScriptMessage.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "browsingContext.contextDestroyed",
        this._onBrowsingContextDestroyed.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
      ...
      _eventsHelper.eventsHelper.addEventListener(
        ...
        this._onNavigationFailed.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "browsingContext.fragmentNavigated",
        this._onFragmentNavigated.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
      ...
      _eventsHelper.eventsHelper.addEventListener(
        ...
        this._onUserPromptOpened.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "log.entryAdded",
        this._onLogEntryAdded.bind(this),
      ),
    ];

    // Initialize main frame.
    this._pagePromise = this._initialize()
      .finally(async () => {
  ...
  async _addAllInitScripts() {
    return Promise.all(
      this._page.allInitScripts().map((initScript) => this.addInitScript(initScript)),
    );
  }
  ...
  _onLogEntryAdded(params) {
    ...
    if (!context) return;
    const callFrame =
      (_params$stackTrace = params.stackTrace) === null || _params$stackTrace === void 0
        ? void 0
        : _params$stackTrace.callFrames[0];
    const location =
      callFrame !== null && callFrame !== void 0
        ? callFrame
        : {
            url: "",
            lineNumber: 1,
  ...
  async goBack() {
    return await this._session
      .send("browsingContext.traverseHistory", {
        context: this._session.sessionId,
        delta: -1,
      })
      .then(() => true)
  ...
  async _installMainBinding() {
    ...
    const promises = [];
    promises.push(
      this._session.send("script.addPreloadScript", {
        functionDeclaration,
        arguments: args,
      }),
    );
    promises.push(
      this._session.send("script.callFunction", {
        functionDeclaration,
        arguments: args,
  ...
  async removeNonInternalInitScripts() {
    const promises = this._initScriptIds.map((script) =>
      this._session.send("script.removePreloadScript", {
        script,
      }),
    );
    this._initScriptIds = [];
  ...
  async scrollRectIntoViewIfNeeded(handle, rect) {
    return await handle
      .evaluateInUtility(([injected, node]) => {
        node.scrollIntoView({
          block: "center",
          inline: "center",
          behavior: "instant",
