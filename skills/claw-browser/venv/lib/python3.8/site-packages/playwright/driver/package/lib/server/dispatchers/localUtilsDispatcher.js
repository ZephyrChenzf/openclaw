...
class LocalUtilsDispatcher extends _dispatcher.Dispatcher {
  constructor(scope, playwright) {
    ...
    const deviceDescriptors = Object.entries(_deviceDescriptors.deviceDescriptors).map(
      ([name, descriptor]) => ({
        name,
        descriptor,
      }),
  ...
  async zip(params) {
    ...
    if (stackSession !== null && stackSession !== void 0 && stackSession.callStacks.length) {
      ...
      if (process.env.PW_LIVE_TRACE_STACKS) {
        ...
        const buffer = Buffer.from(
          JSON.stringify((0, _utils.serializeClientSideCallMetadata)(stackSession.callStacks)),
        );
    ...
    if (params.includeSources) {
      ...
      for (const { stack } of (stackSession === null || stackSession === void 0
        ? void 0
        : stackSession.callStacks) || []) {
        if (!stack) continue;
        for (const { file } of stack) sourceFiles.add(file);
      }
    ...
    if (params.mode === "write") {
      ...
      zipFile.end(undefined, () => {
        zipFile.outputStream
          .pipe(_fs.default.createWriteStream(params.zipFile))
  ...
  async connect(params, metadata) {
    ...
    return await controller.run(async (progress) => {
      ...
      const wsHeaders = {
        ...
        "x-playwright-proxy":
          (_params$exposeNetwork = params.exposeNetwork) !== null &&
          _params$exposeNetwork !== void 0
            ? _params$exposeNetwork
            : "",
        ...params.headers,
      };
      const wsEndpoint = await urlToWSEndpoint(progress, params.wsEndpoint);
      const transport = await _transport.WebSocketTransport.connect(
        progress,
        wsEndpoint,
        wsHeaders,
        true,
        "x-playwright-debug-log",
      );
      const socksInterceptor = new _socksInterceptor.SocksInterceptor(
        transport,
  ...
  async tracingStarted(params, metadata) {
    ...
    if (!params.tracesDir)
      tmpDir = await _fs.default.promises.mkdtemp(
        _path.default.join(_os.default.tmpdir(), "playwright-tracing-"),
      );
    const traceStacksFile = _path.default.join(
      params.tracesDir || tmpDir,
      params.traceName + ".stacks",
    );
    ...
    for (const session of this._stackSessions.values()) {
      ...
      if (process.env.PW_LIVE_TRACE_STACKS) {
        session.writer = session.writer.then(() => {
          const buffer = Buffer.from(
            JSON.stringify((0, _utils.serializeClientSideCallMetadata)(session.callStacks)),
          );
...
class HarBackend {
  ...
  async _harFindResponse(url, method, headers, postData) {
    ...
    while (true) {
        ...
        if (method === "POST" && postData && candidate.request.postData) {
          ...
          if (!buffer.equals(postData)) {
            ...
            if (
              ...
              buffer.toString().replaceAll(candidataBoundary, "")
            )
              continue;
          }
        }
      ...
      if (redirectStatus.includes(entry.response.status) && locationHeader) {
        ...
        if (
          ((entry.response.status === 301 || entry.response.status === 302) && method === "POST") ||
  ...
  dispose() {
    var _this$_zipFile;
    (_this$_zipFile = this._zipFile) === null ||
      _this$_zipFile === void 0 ||
  ...
  const json = await (0, _network.fetchData)(
    {
      ...
      method: "GET",
      timeout:
        (_progress$timeUntilDe =
          progress === null || progress === void 0 ? void 0 : progress.timeUntilDeadline()) !==
          null && _progress$timeUntilDe !== void 0
          ? _progress$timeUntilDe
          : 30_000,
      headers: {
        "User-Agent": (0, _userAgent.getUserAgent)(),
    ...
    async (params, response) => {
      return new Error(
        `Unexpected status ${response.statusCode} when connecting to ${fetchUrl.toString()}.\n` +
...
function multipartBoundary(headers) {
  ...
  if (
    !(
      contentType !== null &&
      contentType !== void 0 &&
      contentType.value.includes("multipart/form-data")
    )
  )
    return undefined;
