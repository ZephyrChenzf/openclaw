...
class RecorderInTraceViewer extends _events.EventEmitter {
  static factory(context) {
    return async (recorder) => {
      ...
      const { wsEndpointForTest, tracePage, traceServer } = await openApp(trace, {
        transport,
...
async function openApp(trace, options) {
  ...
  const page = await (0, _traceViewer.openTraceViewerApp)(
    traceServer.urlPrefix("precise"),
    "chromium",
    options,
  );
  return {
...
class RecorderTransport {
  ...
  deliverEvent(method, params) {
    this._connected.then(() => {
      var _this$sendEvent;
      return (_this$sendEvent = this.sendEvent) === null || _this$sendEvent === void 0
        ? void 0
        : _this$sendEvent.call(this, method, params);
    });
  }
}
