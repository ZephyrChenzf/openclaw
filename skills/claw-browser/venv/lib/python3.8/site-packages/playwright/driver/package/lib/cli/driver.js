...
function runDriver() {
  ...
  new _server.RootDispatcher(dispatcherConnection, async (rootScope, { sdkLanguage }) => {
  ...
  const isJavaScriptLanguageBinding =
    !process.env.PW_LANG_NAME || process.env.PW_LANG_NAME === "javascript";
  const replacer =
    !isJavaScriptLanguageBinding && String.prototype.toWellFormed
      ? (key, value) => {
...
async function runServer(options) {
  const { port, host, path = "/", maxConnections = Infinity, extension } = options;
