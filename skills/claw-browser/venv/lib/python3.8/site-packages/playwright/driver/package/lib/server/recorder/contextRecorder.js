...
class ContextRecorder extends _events.EventEmitter {
  constructor(codegenMode, context, params, delegate) {
    ...
    this._collection.on("change", (actions) => {
      ...
      for (const languageGenerator of this._orderedLanguages) {
        ...
        const { header, footer, actionTexts, text } = (0, _language.generateCode)(
          actions,
          languageGenerator,
        ...
        if (languageGenerator === this._orderedLanguages[0])
          (_this$_throttledOutpu = this._throttledOutputFile) === null ||
            _this$_throttledOutpu === void 0 ||
            _this$_throttledOutpu.setContent(source.text);
      }
    ...
    context.on(_browserContext.BrowserContext.Events.BeforeClose, () => {
      ...
      (_this$_throttledOutpu2 = this._throttledOutputFile) === null ||
        _this$_throttledOutpu2 === void 0 ||
        _this$_throttledOutpu2.flush();
    });
    this._listeners.push(
      _utils.eventsHelper.addEventListener(process, "exit", () => {
        var _this$_throttledOutpu3;
        (_this$_throttledOutpu3 = this._throttledOutputFile) === null ||
          _this$_throttledOutpu3 === void 0 ||
          _this$_throttledOutpu3.flush();
      }),
    );
  ...
  setOutput(codegenId, outputFile) {
    ...
    (_this$_collection = this._collection) === null ||
      _this$_collection === void 0 ||
      _this$_collection.restart();
  }
  languageName(id) {
  ...
  async install() {
    ...
    this._context.on(_browserContext.BrowserContext.Events.Dialog, (dialog) =>
      this._onDialog(dialog.page()),
    );

    // Input actions that potentially lead to navigation are intercepted on the page and are
    // performed by the Playwright.
    await this._context.exposeBinding("__pw_recorderPerformAction", false, (source, action) =>
      this._performAction(source.frame, action),
    );

    // Other non-essential actions are simply being recorded.
    await this._context.exposeBinding("__pw_recorderRecordAction", false, (source, action) =>
  ...
  testIdAttributeName() {
    return (
      this._params.testIdAttributeName ||
      this._context.selectors().testIdAttributeName() ||
      "data-testid"
    );
  }
  async _createActionInContext(frame, action) {
    ...
    await ((_this$_delegate$rewri = (_this$_delegate = this._delegate).rewriteActionInContext) ===
      null || _this$_delegate$rewri === void 0
      ? void 0
      : _this$_delegate$rewri.call(_this$_delegate, this._pageAliases, actionInContext));
...
async function generateFrameSelectorInParent(parent, frame) {
  const result = await (0, _timeoutRunner.raceAgainstDeadline)(
    async () => {
      try {
