...
class Request extends _channelOwner.ChannelOwner {
  ...
  postData() {
    var _ref;
    return (
      ((_ref = this._fallbackOverrides.postDataBuffer || this._initializer.postData) === null ||
      _ref === void 0
        ? void 0
        : _ref.toString("utf-8")) || null
    );
  }
  postDataBuffer() {
  ...
  _safePage() {
    ...
    return (
      ((_Frame$fromNullable = _frame.Frame.fromNullable(this._initializer.frame)) === null ||
      _Frame$fromNullable === void 0
        ? void 0
        : _Frame$fromNullable._page) || null
    );
  }
  serviceWorker() {
    return this._initializer.serviceWorker
      ? _worker.Worker.from(this._initializer.serviceWorker)
      : null;
  }
  isNavigationRequest() {
  ...
  _targetClosedScope() {
    ...
    return (
      ((_this$serviceWorker = this.serviceWorker()) === null || _this$serviceWorker === void 0
        ? void 0
        : _this$serviceWorker._closedScope) ||
      ((_this$_safePage = this._safePage()) === null || _this$_safePage === void 0
        ? void 0
        : _this$_safePage._closedOrCrashedScope) ||
      new _manualPromise.LongStandingScope()
    );
  }
}
...
class Route extends _channelOwner.ChannelOwner {
  ...
  async _redirectNavigationRequest(url) {
    await this._handleRoute(async () => {
      await this._raceWithTargetClose(
        this._channel.redirectNavigationRequest({
          url,
        }),
      );
    });
  ...
  async _innerFulfill(options = {}) {
    ...
    if (options.response instanceof _fetch.APIResponse) {
      ...
      (_statusOption = statusOption) !== null && _statusOption !== void 0
        ? _statusOption
        : (statusOption = options.response.status());
      (_headersOption = headersOption) !== null && _headersOption !== void 0
        ? _headersOption
    ...
    if (options.contentType) headers["content-type"] = String(options.contentType);
      ...
      headers["content-type"] =
        _utilsBundle.mime.getType(options.path) || "application/octet-stream";
...
class WebSocket extends _channelOwner.ChannelOwner {
  ...
  async waitForEvent(event, optionsOrPredicate = {}) {
    return await this._wrapApiCall(async () => {
      ...
      waiter.rejectOnEvent(this._page, _events.Events.Page.Close, () =>
