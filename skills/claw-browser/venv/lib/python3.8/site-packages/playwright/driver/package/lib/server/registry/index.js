...
const registryDirectory = (exports.registryDirectory = (() => {
  ...
  if (envDefined === "0") {
    ...
    if (process.platform === "linux")
      ...
      cacheDirectory =
        process.env.LOCALAPPDATA || _path.default.join(os.homedir(), "AppData", "Local");
...
function readDescriptors(browsersJSON) {
  return browsersJSON["browsers"].map((obj) => {
    ...
    const browserDirectoryPrefix = revisionOverride
      ? `${name}_${_hostPlatform.hostPlatform}_special`
      : `${name}`;
    const descriptor = {
      name,
      revision,
...
class Registry {
  constructor(browsersJSON) {
    ...
    const executablePathOrDie = (name, e, installByDefault, sdkLanguage) => {
      ...
      if (!(0, _fileUtils.canAccessFile)(e)) {
        ...
        const preferredDockerVersion = currentDockerVersion
          ? (0, _dependencies.dockerVersion)(currentDockerVersion.dockerImageNameTemplate)
          : null;
        const isOutdatedDockerImage =
          currentDockerVersion &&
          preferredDockerVersion &&
          currentDockerVersion.dockerImageName !== preferredDockerVersion.dockerImageName;
    ...
    this._executables.push({
      ...
      executablePathOrDie: (sdkLanguage) =>
        executablePathOrDie("chromium", chromiumExecutable, chromium.installByDefault, sdkLanguage),
    ...
    this._executables.push({
      ...
      executablePathOrDie: (sdkLanguage) =>
        executablePathOrDie(
          "chromium-tip-of-tree",
          chromiumTipOfTreeExecutable,
          chromiumTipOfTree.installByDefault,
          sdkLanguage,
        ),
    ...
    this._executables.push(
      this._createBidiFirefoxChannel("bidi-firefox-stable", {
        linux: "/firefox/firefox",
    ...
    this._executables.push(
      this._createBidiFirefoxChannel("bidi-firefox-beta", {
        linux: "/firefox/firefox",
        darwin: "/Firefox.app/Contents/MacOS/firefox",
        win32: "\\core\\firefox.exe",
      }),
    ...
    this._executables.push(
      this._createBidiFirefoxChannel("bidi-firefox-nightly", {
        linux: "/firefox/firefox",
    ...
    this._executables.push(
      this._createBidiChannel("bidi-chrome-stable", {
        linux: "/opt/google/chrome/chrome",
        darwin: "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome",
        win32: `\\Google\\Chrome\\Application\\chrome.exe`,
      }),
    ...
    this._executables.push(
      this._createBidiChannel("bidi-chrome-canary", {
        linux: "",
        darwin: "/Applications/Google Chrome Canary.app/Contents/MacOS/Google Chrome Canary",
        win32: `\\Google\\Chrome SxS\\Application\\chrome.exe`,
      }),
    ...
    this._executables.push({
      ...
      executablePathOrDie: (sdkLanguage) =>
        executablePathOrDie("firefox", firefoxExecutable, firefox.installByDefault, sdkLanguage),
    ...
    this._executables.push({
      ...
      executablePathOrDie: (sdkLanguage) =>
        executablePathOrDie("webkit", webkitExecutable, webkit.installByDefault, sdkLanguage),
    ...
    this._executables.push({
      ...
      executablePathOrDie: (sdkLanguage) =>
        executablePathOrDie("ffmpeg", ffmpegExecutable, ffmpeg.installByDefault, sdkLanguage),
  ...
  _createBidiFirefoxChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      if (found.length > 1) {
        if (shouldThrow)
          throw new Error(
            `Multiple Firefox installations found for channel '${name}': ${found.join(", ")}`,
          );
  ...
  async _validateHostRequirements(
    ...
    if (os.platform() === "win32" && os.arch() === "x64")
      return await (0, _dependencies.validateDependenciesWindows)(
        windowsExeAndDllDirectories.map((d) => _path.default.join(browserDirectory, d)),
      );
  }
  ...
  async install(executablesToInstall, forceReinstall) {
    ...
    try {
      ...
      for (const executable of executables) {
        ...
        const { embedderName } = (0, _userAgent.getEmbedderName)();
        if (
          !(0, _utils.getAsBooleanFromENV)("CI") &&
          !executable._isHermeticInstallation &&
          !forceReinstall &&
          executable.executablePath(embedderName)
        ) {
          const command = buildPlaywrightCLICommand(
            embedderName,
            "install --force " + executable.name,
          );
          throw new Error(
            "\n" +
  ...
  async uninstall(all) {
    ...
    if (all) {
      ...
      await fs.promises
        .unlink(_path.default.join(linksDir, (0, _utils.calculateSha1)(PACKAGE_PATH)))
        .catch(() => {});
    }
    ...
    await this._validateInstallationCache(linksDir);
    return {
      numberOfBrowsersLeft: (await fs.promises.readdir(registryDirectory).catch(() => [])).filter(
        (browserDirectory) => isBrowserDirectory(browserDirectory),
      ).length,
    };
  }
  ...
  async _validateHostRequirementsForExecutableIfNeeded(executable, sdkLanguage) {
    ...
    if (
      await fs.promises
        .stat(markerFile)
        .then((stat) => Date.now() - stat.mtime.getTime() < kMaximumReValidationPeriod)
        .catch(() => false)
    )
      return;
    _debugLogger.debugLogger.log(
      "install",
      `validating host requirements for "${executable.name}"`,
    );
    try {
  ...
  async _downloadExecutable(descriptor, executablePath) {
    ...
    if (!downloadURLs.length)
      throw new Error(
        `ERROR: Playwright does not support ${descriptor.name} on ${_hostPlatform.hostPlatform}`,
      );
    if (!_hostPlatform.isOfficiallySupportedPlatform)
      (0, _browserFetcher.logPolitely)(
        `BEWARE: your OS is not officially supported by Playwright; downloading fallback build for ${_hostPlatform.hostPlatform}.`,
      );
    ...
    const downloadConnectionTimeoutEnv = (0, _utils.getFromENV)(
  ...
  async _installMSEdgeChannel(channel, scripts) {
    const scriptArgs = [];
    if (process.platform !== "linux") {
      const products = lowercaseAllKeys(
        JSON.parse(
      ...
      const artifact = release
        ? release.artifacts.find((artifact) => artifact.artifactname === searchConfig.artifact)
        : null;
      if (artifact) scriptArgs.push(artifact.location /* url */);
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    if (isPowerShell) {
      ...
      const { code } = await (0, _spawnAsync.spawnAsync)("powershell.exe", args, {
        cwd,
        stdio: "inherit",
      });
      if (code !== 0) throw new Error(`Failed to install ${channel}`);
    } else {
      const { command, args, elevatedPermissions } = await (0,
      _dependencies.transformCommandsForRoot)([
        `bash "${_path.default.join(BIN_PATH, scriptName)}" ${scriptArgs.join("")}`,
      ]);
      if (elevatedPermissions) console.log("Switching to root user to install dependencies..."); // eslint-disable-line no-console
      const { code } = await (0, _spawnAsync.spawnAsync)(command, args, {
        cwd,
        stdio: "inherit",
      });
      if (code !== 0) throw new Error(`Failed to install ${channel}`);
  ...
  async _validateInstallationCache(linksDir) {
      ...
      try {
        ...
        for (const browserName of allDownloadable) {
          ...
          const shouldHaveMarkerFile =
            (browserName === "chromium" &&
              (browserRevision >= 786218 || browserRevision < 300000)) ||
          ...
          if (
            ...
            (await (0, _fileUtils.existsAsync)(browserDirectoryToMarkerFilePath(usedBrowserPath)))
          )
    ...
    if (!(0, _utils.getAsBooleanFromENV)("PLAYWRIGHT_SKIP_BROWSER_GC")) {
      let downloadedBrowsers = (await fs.promises.readdir(registryDirectory)).map((file) =>
        _path.default.join(registryDirectory, file),
      );
