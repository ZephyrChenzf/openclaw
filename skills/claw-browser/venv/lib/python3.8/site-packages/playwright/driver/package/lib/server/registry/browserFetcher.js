...
async function downloadBrowserWithProgressBar(
  ...
  if (
    await (0, _fileUtils.existsAsync)((0, _.browserDirectoryToMarkerFilePath)(browserDirectory))
  ) {
    // Already downloaded.
  ...
  try {
    ...
    for (let attempt = 1; attempt <= retryCount; ++attempt) {
      ...
      const { error } = await downloadBrowserWithProgressBarOutOfProcess(
        title,
        browserDirectory,
        url,
        zipPath,
...
function downloadBrowserWithProgressBarOutOfProcess(
  ...
  const cp = _child_process.default.fork(
    _path.default.join(__dirname, "oopDownloadBrowserMain.js"),
  );
  ...
  cp.on("message", (message) => {
    ...
    if ((message === null || message === void 0 ? void 0 : message.method) === "progress")
  ...
  cp.on("exit", (code) => {
    ...
    if (!_fs.default.existsSync((0, _.browserDirectoryToMarkerFilePath)(browserDirectory)))
      promise.resolve({
        error: new Error(
          `Download failure, ${(0, _.browserDirectoryToMarkerFilePath)(browserDirectory)} does not exist`,
        ),
      });
    else
      promise.resolve({
