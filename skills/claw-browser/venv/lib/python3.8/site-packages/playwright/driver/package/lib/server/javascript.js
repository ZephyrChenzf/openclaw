...
class ExecutionContext extends _instrumentation.SdkObject {
  ...
  evaluateWithArguments(expression, returnByValue, utilityScript, values, objectIds) {
    return this._raceAgainstContextDestroyed(
      this._delegate.evaluateWithArguments(
        expression,
        returnByValue,
        utilityScript,
        values,
  ...
  utilityScript() {
    if (!this._utilityScriptPromise) {
      ...
      this._utilityScriptPromise = this._raceAgainstContextDestroyed(
        this._delegate
          .rawEvaluateHandle(source)
          .then((objectId) => new JSHandle(this, "object", "UtilityScript", objectId)),
      );
    }
...
class JSHandle extends _instrumentation.SdkObject {
  ...
  async jsonValue() {
    ...
    return this._context.evaluateWithArguments(
      script,
      true,
      utilityScript,
      [true],
...
async function evaluateExpression(context, expression, options, ...args) {
  ...
  try {
    return await context.evaluateWithArguments(
      script,
