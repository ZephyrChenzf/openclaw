...
exports.GlobalAPIRequestContext =
  exports.BrowserContextAPIRequestContext =
...
class APIRequestContext extends _instrumentation.SdkObject {
  ...
  async fetch(params, metadata) {
    ...
    if ((credentials === null || credentials === void 0 ? void 0 : credentials.send) === "always")
    ...
    const options = {
      ...
      maxRedirects:
        params.maxRedirects === 0
          ? -1
          : params.maxRedirects === undefined
            ? 20
            : params.maxRedirects,
      timeout,
      deadline,
      ...(0, _socksClientCertificatesInterceptor.getMatchingTLSOptionsForOrigin)(
    ...
    const fetchResponse = await controller.run((progress) => {
      return this._sendRequestWithRetries(
        progress,
        requestUrl,
        options,
        postData,
  ...
  async _sendRequestWithRetries(progress, url, options, postData, maxRetries) {
    ...
    for (let i = 0; i <= maxRetries; i++) {
      try {
        ...
        if (
          i === maxRetries ||
          (options.deadline && (0, _utils.monotonicTime)() + backoff > options.deadline)
        )
  ...
  async _sendRequest(progress, url, options, postData) {
    ...
    const requestCookies =
      ((_getHeader = getHeader(options.headers, "cookie")) === null || _getHeader === void 0
        ? void 0
        : _getHeader.split(";").map((p) => {
    ...
    return new Promise((fulfill, reject) => {
      const requestConstructor = (url.protocol === "https:" ? _https.default : _http.default)
        .request;
      // If we have a proxy agent already, do not override it.
      const agent =
        options.agent ||
        (url.protocol === "https:"
          ? _happyEyeballs.httpsHappyEyeballsAgent
          : _happyEyeballs.httpHappyEyeballsAgent);
      ...
      const request = requestConstructor(url, requestOptions, async (response) => {
        ...
        const notifyRequestFinished = (body) => {
          ...
          const timings = {
            ...
            connect:
              ((_tlsHandshakeAt = tlsHandshakeAt) !== null && _tlsHandshakeAt !== void 0
                ? _tlsHandshakeAt
        ...
        if (redirectStatus.includes(response.statusCode) && options.maxRedirects >= 0) {
          ...
          const locationHeaderValue = Buffer.from(
            (_response$headers$loc = response.headers.location) !== null &&
              _response$headers$loc !== void 0
              ? _response$headers$loc
              : "",
            "latin1",
          ).toString("utf8");
      ...
      listeners.push(
        _utils.eventsHelper.addEventListener(this, APIRequestContext.Events.Dispose, () => {
      ...
      request.on("socket", (socket) => {
        ...
        listeners.push(
          _utils.eventsHelper.addEventListener(socket, "lookup", () => {
          ...
          _utils.eventsHelper.addEventListener(socket, "secureConnect", () => {
            ...
            if (socket instanceof _tls.TLSSocket) {
              ...
              securityDetails = {
                protocol:
                  (_socket$getProtocol = socket.getProtocol()) !== null &&
                  _socket$getProtocol !== void 0
                    ? _socket$getProtocol
                    : undefined,
  ...
  _getHttpCredentials(url) {
    ...
    if (
      !(
        (_this$_defaultOptions = this._defaultOptions().httpCredentials) !== null &&
        _this$_defaultOptions !== void 0 &&
        _this$_defaultOptions.origin
      ) ||
      url.origin.toLowerCase() ===
        ((_this$_defaultOptions2 = this._defaultOptions().httpCredentials) === null ||
        _this$_defaultOptions2 === void 0 ||
        (_this$_defaultOptions2 = _this$_defaultOptions2.origin) === null ||
        _this$_defaultOptions2 === void 0
          ? void 0
          : _this$_defaultOptions2.toLowerCase())
    )
