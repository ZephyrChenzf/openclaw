...
function loadDummyServerCertsIfNeeded() {
  ...
  const { cert, key } = (0, _utils.generateSelfSignedCertificate)();
...
class SocksProxyConnection {
  constructor(socksProxy, uid, host, port) {
    ...
    this._targetCloseEventListener = () => {
      ...
      (_this$internalTLS = this.internalTLS) === null ||
        _this$internalTLS === void 0 ||
        _this$internalTLS.destroy();
      (_this$_dummyServer = this._dummyServer) === null ||
        _this$_dummyServer === void 0 ||
        _this$_dummyServer.close();
    };
  }
  async connect() {
    if (this.socksProxy.proxyAgentFromOptions)
      this.target = await this.socksProxy.proxyAgentFromOptions.callback(
      ...
      this.target = await (0, _happyEyeballs.createSocket)(
        rewriteToLocalhostIfNeeded(this.host),
        this.port,
      );
    this.target.once("close", this._targetCloseEventListener);
    this.target.once("error", (error) =>
      this.socksProxy._socksProxy.sendSocketError({
        uid: this.uid,
        error: error.message,
      }),
    );
  ...
  onClose() {
    ...
    // Close the other end and cleanup TLS resources.
    this.target.destroy();
    (_this$internalTLS2 = this.internalTLS) === null ||
      _this$internalTLS2 === void 0 ||
      _this$internalTLS2.destroy();
    (_this$_dummyServer2 = this._dummyServer) === null ||
      _this$_dummyServer2 === void 0 ||
      _this$_dummyServer2.close();
    this._closed = true;
  }
  onData(data) {
    ...
    if (!this.firstPackageReceived) {
      ...
      if (data[0] === 0x16) this._attachTLSListeners();
      else
  ...
  _attachTLSListeners() {
    ...
    this.socksProxy.alpnCache.get(
      rewriteToLocalhostIfNeeded(this.host),
      this.port,
      (alpnProtocolChosenByServer) => {
        _debugLogger.debugLogger.log(
          "client-certificates",
          `Proxy->Target ${this.host}:${this.port} chooses ALPN ${alpnProtocolChosenByServer}`,
        );
        ...
        this._dummyServer.once("secureConnection", (internalTLS) => {
          ...
          _debugLogger.debugLogger.log(
            "client-certificates",
            `Browser->Proxy ${this.host}:${this.port} chooses ALPN ${internalTLS.alpnProtocol}`,
          );
          let targetTLS = undefined;
          const handleError = (error) => {
            _debugLogger.debugLogger.log(
              "client-certificates",
              `error when connecting to target: ${error.message.replaceAll("\n", " ")}`,
            );
            const responseBody = (0, _utils.escapeHTML)(
              "Playwright client-certificate error: " + error.message,
            ).replaceAll("\n", " <br>");
            if (
              (internalTLS === null || internalTLS === void 0
                ? void 0
                : internalTLS.alpnProtocol) === "h2"
            ) {
          ...
          targetTLS = _tls.default.connect({
            ...
            secureContext: this.socksProxy.secureContextMap.get(
              new URL(`https://${this.host}:${this.port}`).origin,
            ),
          });
...
class ClientCertificatesProxy {
  constructor(contextOptions) {
    ...
    this.proxyAgentFromOptions = contextOptions.proxy
      ? (0, _fetch.createProxyAgent)(contextOptions.proxy)
      : undefined;
    ...
    this._socksProxy.addListener(_socksProxy.SocksProxy.Events.SocksData, async (payload) => {
      ...
      (_this$_connections$ge = this._connections.get(payload.uid)) === null ||
        _this$_connections$ge === void 0 ||
        _this$_connections$ge.onData(payload.data);
    });
    this._socksProxy.addListener(_socksProxy.SocksProxy.Events.SocksClosed, (payload) => {
      var _this$_connections$ge2;
      (_this$_connections$ge2 = this._connections.get(payload.uid)) === null ||
        _this$_connections$ge2 === void 0 ||
        _this$_connections$ge2.onClose();
  ...
  _initSecureContexts(clientCertificates) {
    ...
    for (const [origin, certs] of origin2certs) {
      try {
        this.secureContextMap.set(
          origin,
          _tls.default.createSecureContext(convertClientCertificatesToTLSOptions(certs)),
        );
      } catch (error) {
        error = rewriteOpenSSLErrorIfNeeded(error);
        throw (0, _utils.rewriteErrorMessage)(
          error,
...
function getMatchingTLSOptionsForOrigin(clientCertificates, origin) {
  const matchingCerts =
    clientCertificates === null || clientCertificates === void 0
      ? void 0
      : clientCertificates.filter((c) => normalizeOrigin(c.origin) === origin);
...
function rewriteOpenSSLErrorIfNeeded(error) {
  if (error.message !== "unsupported" && error.code !== "ERR_CRYPTO_UNSUPPORTED_OPERATION")
    return error;
  return (0, _utils.rewriteErrorMessage)(
    error,
    [
