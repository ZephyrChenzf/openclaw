...
class Page extends _instrumentation.SdkObject {
  ...
  async exposeBinding(name, needsHandle, playwrightBinding) {
    ...
    await Promise.all(
      this.frames().map((frame) =>
        frame.evaluateExpression(binding.initScript.source).catch((e) => {}),
      ),
    );
  }
  ...
  async reload(metadata, options) {
    ...
    return controller.run(
      (progress) =>
        this.mainFrame().raceNavigationAction(progress, options, async () => {
      ...
      (progress) =>
        this.mainFrame().raceNavigationAction(progress, options, async () => {
          ...
          const waitPromise = this.mainFrame()
            ._waitForNavigation(progress, false /* requiresNewDocument */, options)
            .catch((e) => {
              error = e;
              return null;
            });
  ...
  resolveLocatorHandler(uid, remove) {
    ...
    if (handler) {
      ...
      (_handler$resolved = handler.resolved) === null ||
        _handler$resolved === void 0 ||
        _handler$resolved.resolve();
  ...
  async performLocatorHandlersCheckpoint(progress) {
    ...
    for (const [uid, handler] of this._locatorHandlers) {
      if (!handler.resolved) {
        if (
          await this.mainFrame().isVisibleInternal(handler.selector, {
            strict: true,
          })
        ) {
      ...
      if (handler.resolved) {
        ...
        progress.log(
          `  found ${(0, _utils.asLocator)(this.attribution.playwright.options.sdkLanguage, handler.selector)}, intercepting action to run the handler`,
        );
        const promise = handler.resolved.then(async () => {
          progress.throwIfAborted();
          if (!handler.noWaitAfter) {
            progress.log(
              `  locator handler has finished, waiting for ${(0, _utils.asLocator)(this.attribution.playwright.options.sdkLanguage, handler.selector)} to be hidden`,
            );
  ...
  emulatedMedia() {
    ...
    return {
      ...
      colorScheme:
        this._emulatedMedia.colorScheme !== undefined
          ? this._emulatedMedia.colorScheme
          : (_contextOptions$color = contextOptions.colorScheme) !== null &&
              _contextOptions$color !== void 0
            ? _contextOptions$color
            : "light",
      reducedMotion:
        this._emulatedMedia.reducedMotion !== undefined
          ? this._emulatedMedia.reducedMotion
          : (_contextOptions$reduc = contextOptions.reducedMotion) !== null &&
              _contextOptions$reduc !== void 0
            ? _contextOptions$reduc
            : "no-preference",
      forcedColors:
        this._emulatedMedia.forcedColors !== undefined
          ? this._emulatedMedia.forcedColors
          : (_contextOptions$force = contextOptions.forcedColors) !== null &&
              _contextOptions$force !== void 0
            ? _contextOptions$force
            : "none",
    };
  }
  ...
  viewportSize() {
    ...
    return (
      ((_this$emulatedSize = this.emulatedSize()) === null || _this$emulatedSize === void 0
        ? void 0
        : _this$emulatedSize.viewport) || null
    );
  }
  emulatedSize() {
  ...
  async expectScreenshot(metadata, options = {}) {
    ...
    const areEqualScreenshots = (actual, expected, previous) => {
      const comparatorResult =
        actual && expected ? comparator(actual, expected, options) : undefined;
    ...
    return controller
      .run(async (progress) => {
        ...
        if (options.expected) progress.log(`  verifying given screenshot expectation`);
        else progress.log(`  generating new stable screenshot expectation`);
        ...
        while (true) {
          ...
          const screenshotTimeout =
            (_pollIntervals$shift = pollIntervals.shift()) !== null &&
            _pollIntervals$shift !== void 0
              ? _pollIntervals$shift
              : 1000;
  ...
  allInitScripts() {
    ...
    return [
      ...bindings.map((binding) => binding.initScript),
  ...
  static async dispatch(page, payload, context) {
    ...
    try {
      ...
      if (binding.needsHandle) {
        ...
        const args = serializedArgs.map((a) =>
          (0, _utilityScriptSerializers.parseEvaluationResultValue)(a),
        );
        result = await binding.playwrightFunction(
          {
            frame: context.frame,
