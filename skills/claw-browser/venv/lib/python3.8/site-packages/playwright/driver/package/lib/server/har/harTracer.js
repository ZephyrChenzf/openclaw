...
class HarTracer {
  constructor(context, page, delegate, options) {
    ...
    this._baseURL =
      context instanceof _fetch.APIRequestContext
        ? context._defaultOptions().baseURL
  ...
  start(options) {
    ...
    this._started = true;
    const apiRequest =
      this._context instanceof _fetch.APIRequestContext
        ? this._context
        : this._context.fetchRequest;
    this._eventListeners = [
      _eventsHelper.eventsHelper.addEventListener(
        apiRequest,
        _fetch.APIRequestContext.Events.Request,
        (event) => this._onAPIRequest(event),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        apiRequest,
        _fetch.APIRequestContext.Events.RequestFinished,
        (event) => this._onAPIRequestFinished(event),
      ),
    ];
    if (this._context instanceof _browserContext.BrowserContext) {
      this._eventListeners.push(
        _eventsHelper.eventsHelper.addEventListener(
          ...
          (page) => this._createPageEntryIfNeeded(page),
        ),
        _eventsHelper.eventsHelper.addEventListener(
          this._context,
        ...
        _eventsHelper.eventsHelper.addEventListener(
          ...
          ({ request, response }) => this._onRequestFinished(request, response).catch(() => {}),
        ),
        _eventsHelper.eventsHelper.addEventListener(
          this._context,
          _browserContext.BrowserContext.Events.RequestFailed,
          (request) => this._onRequestFailed(request),
        ),
        _eventsHelper.eventsHelper.addEventListener(
          this._context,
        ...
        _eventsHelper.eventsHelper.addEventListener(
          this._context,
          _browserContext.BrowserContext.Events.RequestAborted,
          (request) => this._onRequestAborted(request),
        ),
        _eventsHelper.eventsHelper.addEventListener(
          this._context,
          _browserContext.BrowserContext.Events.RequestFulfilled,
          (request) => this._onRequestFulfilled(request),
        ),
        _eventsHelper.eventsHelper.addEventListener(
          this._context,
          _browserContext.BrowserContext.Events.RequestContinued,
          (request) => this._onRequestContinued(request),
        ),
      );
    }
  }
  _shouldIncludeEntryWithUrl(urlString) {
    return (
      !this._options.urlFilter ||
      (0, _utils.urlMatches)(this._baseURL, urlString, this._options.urlFilter)
    );
  }
  ...
  _onAPIRequest(event) {
    ...
    harEntry.request.postData = this._postDataForBuffer(
      event.postData || null,
      event.headers["content-type"],
      this._options.content,
    );
    if (!this._options.omitSizes)
      harEntry.request.bodySize =
        ((_event$postData = event.postData) === null || _event$postData === void 0
          ? void 0
          : _event$postData.length) || 0;
  ...
  _onAPIRequestFinished(event) {
    ...
    harEntry.response.cookies = this._options.omitCookies
      ? []
      : event.cookies.map((c) => {
          return {
            ...c,
    ...
    if (!this._options.omitSizes)
      harEntry.response.bodySize =
        (_event$body$length =
          (_event$body = event.body) === null || _event$body === void 0
            ? void 0
            : _event$body.length) !== null && _event$body$length !== void 0
          ? _event$body$length
          : 0;
    ...
    const promise = response
      ...
      .then((buffer) => {
        if (this._options.omitScripts && request.resourceType() === "script") {
          compressionCalculationBarrier === null ||
            compressionCalculationBarrier === void 0 ||
  ...
  _storeResponseContent(buffer, content, resourceType) {
    ...
    if (this._options.content === "embed") {
      ...
      const sha1 =
        ...
        "." +
        (_utilsBundle.mime.getExtension(content.mimeType) || "dat");
    ...
    if (!this._options.omitTiming) {
      ...
      const dns =
        timing.domainLookupEnd !== -1
          ? _helper.helper.millisToRoundishMillis(timing.domainLookupEnd - timing.domainLookupStart)
          : -1;
      const connect =
        timing.connectEnd !== -1
          ? _helper.helper.millisToRoundishMillis(timing.connectEnd - timing.connectStart)
          : -1;
      const ssl =
        timing.connectEnd !== -1
          ? _helper.helper.millisToRoundishMillis(timing.connectEnd - timing.secureConnectionStart)
          : -1;
      const wait =
        timing.responseStart !== -1
          ? _helper.helper.millisToRoundishMillis(timing.responseStart - timing.requestStart)
          : -1;
      const receive = -1;
      harEntry.timings = {
        dns,
  ...
  stop() {
    ...
    const context =
      this._context instanceof _browserContext.BrowserContext ? this._context : undefined;
    const log = {
      version: "1.2",
      creator: {
        name: "Playwright",
      ...
      browser: {
        name:
          (context === null || context === void 0 ? void 0 : context._browser.options.name) || "",
        version:
          (context === null || context === void 0 ? void 0 : context._browser.version()) || "",
      },
    ...
    if (!this._options.omitTiming) {
      for (const pageEntry of log.pages || []) {
        ...
        if (
          typeof pageEntry.pageTimings.onContentLoad === "number" &&
          pageEntry.pageTimings.onContentLoad >= 0
        )
  ...
  _postDataForBuffer(postData, contentType, content) {
    ...
    if (content === "attach") {
      const sha1 =
        ...
        "." +
        (_utilsBundle.mime.getExtension(contentType) || "dat");
