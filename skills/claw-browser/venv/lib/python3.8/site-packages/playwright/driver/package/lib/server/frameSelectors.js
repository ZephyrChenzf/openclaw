...
class FrameSelectors {
  ...
  _parseSelector(selector, options) {
    const strict =
      typeof (options === null || options === void 0 ? void 0 : options.strict) === "boolean"
        ? options.strict
        : !!this.frame._page.context()._options.strictSelectors;
  ...
  async query(selector, options, scope) {
    ...
    const handle = await resolved.injected.evaluateHandle(
      (injected, { info, scope }) => {
  ...
  async queryArrayInMainWorld(selector, scope) {
    const resolved = await this.resolveInjectedForSelector(
      selector,
      {
        mainWorld: true,
      },
    ...
    return await resolved.injected.evaluateHandle(
      (injected, { info, scope }) => {
  ...
  async queryAll(selector, scope) {
    ...
    // Be careful, |this.frame| can be different from |resolved.frame|.
    if (!resolved) return [];
    const arrayHandle = await resolved.injected.evaluateHandle(
      (injected, { info, scope }) => {
        return injected.querySelectorAll(info.parsed, scope || document);
      },
      {
  ...
  async resolveFrameForSelector(selector, options = {}, scope) {
    ...
    for (const chunk of frameChunks) {
      (0, _selectorParser.visitAllSelectorParts)(chunk, (part, nested) => {
        if (nested && part.name === "internal:control" && part.body === "enter-frame") {
          const locator = (0, _utils.asLocator)(
            this.frame._page.attribution.playwright.options.sdkLanguage,
            selector,
          );
          throw new _selectorParser.InvalidSelectorError(
    ...
    for (let i = 0; i < frameChunks.length - 1; ++i) {
      ...
      const handle = await injectedScript.evaluateHandle(
        (injected, { info, scope, selectorString }) => {
          ...
          if (element && element.nodeName !== "IFRAME" && element.nodeName !== "FRAME")
            throw injected.createStacklessError(
              `Selector "${selectorString}" resolved to ${injected.previewNode(element)}, <iframe> was expected`,
            );
          return element;
        },
        {
  ...
  async resolveInjectedForSelector(selector, options, scope) {
    ...
    // Be careful, |this.frame| can be different from |resolved.frame|.
    if (!resolved) return;
    const context = await resolved.frame._context(
      options !== null && options !== void 0 && options.mainWorld ? "main" : resolved.info.world,
    );
