...
class PlaywrightConnection {
  constructor(lock, clientType, ws, options, preLaunched, id, onClose) {
      ...
      if (ws.readyState !== ws.CLOSING) {
        ...
        if (_debugLogger.debugLogger.isEnabled("server:channel"))
          _debugLogger.debugLogger.log(
            "server:channel",
            `[${this._id}] ${(0, _utils.monotonicTime)() * 1000} SEND ► ${messageString}`,
          );
    ...
    ws.on("message", async (message) => {
      ...
      if (_debugLogger.debugLogger.isEnabled("server:channel"))
        _debugLogger.debugLogger.log(
          "server:channel",
          `[${this._id}] ${(0, _utils.monotonicTime)() * 1000} ◀ RECV ${messageString}`,
        );
      if (_debugLogger.debugLogger.isEnabled("server:metadata"))
    ...
    this._root = new _server.RootDispatcher(this._dispatcherConnection, async (scope, options) => {
      ...
      if (clientType === "pre-launched-browser-or-android")
        return this._preLaunched.browser
          ? await this._initPreLaunchedBrowserMode(scope)
          : await this._initPreLaunchedAndroidMode(scope);
  ...
  async _initLaunchBrowserMode(scope, options) {
    _debugLogger.debugLogger.log(
      "server",
      `[${this._id}] engaged launch mode for "${this._options.browserName}"`,
    );
    ...
    const browser = await playwright[this._options.browserName].launch(
  ...
  async _initPreLaunchedBrowserMode(scope) {
    ...
    (_this$_preLaunched$so = this._preLaunched.socksProxy) === null ||
      _this$_preLaunched$so === void 0 ||
      _this$_preLaunched$so.setPattern(this._options.socksProxyPattern);
    ...
    const playwrightDispatcher = new _server.PlaywrightDispatcher(
      scope,
      playwright,
      this._preLaunched.socksProxy,
      browser,
    );
  ...
  _initDebugControllerMode() {
    ...
    return new _debugControllerDispatcher.DebugControllerDispatcher(
      this._dispatcherConnection,
      playwright.debugController,
    );
  }
  async _initReuseBrowsersMode(scope) {
    ...
    _debugLogger.debugLogger.log(
      "server",
      `[${this._id}] engaged reuse browsers mode for ${this._options.browserName}`,
    );
    const playwright = this._preLaunched.playwright;
    ...
    for (const b of playwright.allBrowsers()) {
      ...
      if (
        b.options.name === this._options.browserName &&
        b.options.channel === this._options.launchOptions.channel
      )
        await b.close({
          reason: "Connection terminated",
        });
    ...
    if (!browser) {
      browser = await playwright[this._options.browserName || "chromium"].launch(
        (0, _instrumentation.serverSideCallMetadata)(),
        {
          ...this._options.launchOptions,
  ...
  async _createOwnedSocksProxy(playwright) {
    ...
    _debugLogger.debugLogger.log(
      "server",
      `[${this._id}] started socks proxy on port ${playwright.options.socksProxyPort}`,
    );
  ...
  logServerMetadata(message, messageString, direction) {
    ...
    _debugLogger.debugLogger.log(
      "server:metadata",
      (direction === "SEND" ? "SEND ► " : "◀ RECV ") + JSON.stringify(serverLogMetadata),
    );
  }
  async close(reason) {
    ...
    _debugLogger.debugLogger.log(
      "server",
      `[${this._id}] force closing connection: ${(reason === null || reason === void 0 ? void 0 : reason.reason) || ""} (${(reason === null || reason === void 0 ? void 0 : reason.code) || 0})`,
    );
    try {
      this._ws.close(
        reason === null || reason === void 0 ? void 0 : reason.code,
        reason === null || reason === void 0 ? void 0 : reason.reason,
      );
    } catch (e) {}
  }
}
