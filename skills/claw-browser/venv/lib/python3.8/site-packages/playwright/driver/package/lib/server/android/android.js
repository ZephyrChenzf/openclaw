...
class AndroidDevice extends _instrumentation.SdkObject {
  ...
  async _installDriver() {
    ...
    ).catch((e) => (0, _utilsBundle.debug)("pw:android")(e));
  ...
  async launchBrowser(pkg = "com.android.chrome", options) {
    ...
    const socketName = (0, _utils.isUnderTest)()
      ? "webview_devtools_remote_playwright_test"
      : "playwright_" + (0, _utils.createGuid)() + "_devtools_remote";
    ...
    await this._backend.runCommand(
      `shell:echo "${Buffer.from(commandLine).toString("base64")}" | base64 -d > /data/local/tmp/chrome-command-line`,
    );
    await this._backend.runCommand(
      `shell:am start -a android.intent.action.VIEW -d about:blank ${pkg}`,
    );
  ...
  async _connectToBrowser(socketName, options = {}) {
    ...
    const cleanupArtifactsDir = async () => {
      ...
      for (let i = 0; i < (errors || []).length; ++i)
        (0, _utilsBundle.debug)("pw:android")(
          `exception while removing ${artifactsDir}: ${errors[i]}`,
        );
    };
    ...
    socket.on("close", async () => {
      ...
      cleanupArtifactsDir().catch((e) =>
        (0, _utilsBundle.debug)("pw:android")(`could not cleanup artifacts dir: ${e}`),
      );
    });
