...
class PageDispatcher extends _dispatcher.Dispatcher {
  ...
  constructor(parentScope, page) {
    ...
    this.addObjectListener(_page.Page.Events.FileChooser, (fileChooser) =>
    ...
    this.addObjectListener(_page.Page.Events.FrameAttached, (frame) =>
      this._onFrameAttached(frame),
    );
    this.addObjectListener(_page.Page.Events.FrameDetached, (frame) =>
      this._onFrameDetached(frame),
    );
    this.addObjectListener(_page.Page.Events.LocatorHandlerTriggered, (uid) =>
      this._dispatchEvent("locatorHandlerTriggered", {
        uid,
      }),
    );
    this.addObjectListener(_page.Page.Events.WebSocket, (webSocket) =>
      this._dispatchEvent("webSocket", {
        webSocket: new _networkDispatchers.WebSocketDispatcher(this, webSocket),
      }),
    );
    this.addObjectListener(_page.Page.Events.Worker, (worker) =>
      this._dispatchEvent("worker", {
        worker: new WorkerDispatcher(this, worker),
      }),
    );
    this.addObjectListener(_page.Page.Events.Video, (artifact) =>
    ...
    await this._page.setClientRequestInterceptor((route, request) => {
      ...
      this._dispatchEvent("route", {
        route: _networkDispatchers.RouteDispatcher.from(
          _networkDispatchers.RequestDispatcher.from(this.parentScope(), request),
          route,
        ),
      });
      return true;
  ...
  async setWebSocketInterceptionPatterns(params, metadata) {
    ...
    if (params.patterns.length)
      await _webSocketRouteDispatcher.WebSocketRouteDispatcher.installIfNeeded(
        this.parentScope(),
        this._page,
      );
  }
  ...
  constructor(scope, name, needsHandle, source, args) {
    super(
      ...
      "BindingCall",
      {
        ...
        handle: needsHandle
          ? _elementHandlerDispatcher.ElementHandleDispatcher.fromJSHandle(scope, args[0])
          : undefined,
      },
    );
