...
_utilsBundle.program
  .command("mark-docker-image [dockerImageNameTemplate]", {
    hidden: true,
  })
...
commandWithOpenOptions("codegen [url]", "open page and generate code for user actions", [
  ...
  [
    "--target <language>",
    `language to generate, one of javascript, playwright-test, python, python-async, python-pytest, csharp, csharp-mstest, csharp-nunit, java, java-junit`,
    codegenId(),
  ],
  ["--save-trace <filename>", "record a trace for the session and save it to a file"],
  [
    "--test-id-attribute <attributeName>",
    "use the specified attribute to generate data test ID selectors",
  ],
])
  .action(function (url, options) {
...
function checkBrowsersToInstall(args) {
  ...
  if (faultyArguments.length)
    throw new Error(
      `Invalid installation targets: ${faultyArguments.map((name) => `'${name}'`).join(", ")}. Expecting one of: ${suggestedBrowsersToInstall()}`,
    );
  return executables;
}
_utilsBundle.program
    ...
    try {
      ...
      if (options.dryRun) {
        for (const executable of executables) {
          ...
          console.log(
            `  Install location:    ${(_executable$directory = executable.directory) !== null && _executable$directory !== void 0 ? _executable$directory : "<system>"}`,
          );
          if (
            (_executable$downloadU = executable.downloadURLs) !== null &&
            _executable$downloadU !== void 0 &&
            _executable$downloadU.length
          ) {
...
_utilsBundle.program
  .command("uninstall")
  .description(
    "Removes browsers used by this installation of Playwright from the system (chromium, firefox, webkit, ffmpeg). This does not include branded channels.",
  )
  .option("--all", "Removes all browsers used by any Playwright installation from the system.")
  .action(async (options) => {
    delete process.env.PLAYWRIGHT_SKIP_BROWSER_GC;
    await _server.registry
      .uninstall(!!options.all)
      .then(({ numberOfBrowsersLeft }) => {
        if (!options.all && numberOfBrowsersLeft > 0) {
          console.log(
            "Successfully uninstalled Playwright browsers for the current Playwright installation.",
          );
          console.log(
            `There are still ${numberOfBrowsersLeft} browsers left, used by other Playwright installations.\nTo uninstall Playwright browsers for all installations, re-run with --all flag.`,
          );
        }
      })
...
_utilsBundle.program
  ...
  .action(async function (args, options) {
    try {
      if (!args.length)
        await _server.registry.installDeps(_server.registry.defaultExecutables(), !!options.dryRun);
      else await _server.registry.installDeps(checkBrowsersToInstall(args), !!options.dryRun);
    } catch (e) {
...
_utilsBundle.program
  ...
  .option(
    "-h, --host <host>",
    "Host to serve trace on; specifying this option opens trace in a browser tab",
  )
  .option(
    "-p, --port <port>",
    "Port to serve trace on, 0 for any free port; specifying this option opens trace in a browser tab",
  )
  ...
  .action(function (traces, options) {
    ...
    if (options.port !== undefined || options.host !== undefined)
      (0, _traceViewer.runTraceInBrowser)(traces, openOptions).catch(logErrorAndExit);
    else
      (0, _traceViewer.runTraceViewerApp)(traces, options.browser, openOptions, true).catch(
        logErrorAndExit,
      );
  })
  .addHelpText(
...
async function launchContext(options, extraOptions) {
  ...
  if (process.env.PWTEST_CLI_IS_UNDER_TEST) {
    ...
    browser.on("disconnected", () => {
      const hasCrashLine = logs.some(
        (line) =>
          line.includes("process did exit:") &&
          !line.includes("process did exit: exitCode=0, signal=null"),
      );
      if (hasCrashLine) {
...
async function codegen(options, url) {
  ...
  const tracesDir = _path.default.join(
    _os.default.tmpdir(),
...
function commandWithOpenOptions(command, description, options) {
  ...
  return result
    ...
    .option(
      "--save-har-glob <glob pattern>",
      "filter entries in the HAR by matching url against this glob pattern",
    )
    .option(
    ...
    .option(
      "--timeout <timeout>",
      "timeout for Playwright actions in milliseconds, no timeout by default",
    )
