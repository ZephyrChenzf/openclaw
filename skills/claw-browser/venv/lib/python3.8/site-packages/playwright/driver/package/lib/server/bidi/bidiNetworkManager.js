...
class BidiNetworkManager {
  constructor(bidiSession, page, onNavigationResponseStarted) {
    ...
    this._eventListeners = [
      _eventsHelper.eventsHelper.addEventListener(
        ...
        this._onBeforeRequestSent.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "network.responseStarted",
        this._onResponseStarted.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "network.responseCompleted",
        this._onResponseCompleted.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
        "network.fetchError",
        this._onFetchError.bind(this),
      ),
      _eventsHelper.eventsHelper.addEventListener(
        bidiSession,
  ...
  _onBeforeRequestSent(param) {
    ...
    const redirectedFrom = param.redirectCount
      ? this._requests.get(param.request.request) || null
      : null;
    const frame = redirectedFrom
      ? redirectedFrom.request.frame()
      : param.context
        ? this._page._frameManager.frame(param.context)
        : null;
    if (!frame) return;
    ...
    if (param.intercepts) {
      ...
      if (redirectedFrom) {
        ...
        this._session.sendMayFail("network.continueRequest", {
          ...
          ...(((_redirectedFrom$_orig = redirectedFrom._originalRequestRoute) === null ||
          _redirectedFrom$_orig === void 0
            ? void 0
            : _redirectedFrom$_orig._alreadyContinuedHeaders) || {}),
        });
      } else {
  ...
  _onResponseStarted(params) {
    ...
    const response = new network.Response(
      ...
      fromBidiHeaders(params.response.headers),
      timing,
      getResponseBody,
      false,
    );
  ...
  _onFetchError(params) {
    ...
    this._page._frameManager.requestFailed(
      request.request,
      params.errorText === "NS_BINDING_ABORTED",
    );
  }
  _onAuthRequired(params) {
    var _params$response$auth;
    const isBasic =
      (_params$response$auth = params.response.authChallenges) === null ||
      _params$response$auth === void 0
        ? void 0
        : _params$response$auth.some((challenge) => challenge.scheme.startsWith("Basic"));
  ...
  async _updateProtocolRequestInterception(initial) {
    ...
    if (enabled) {
      interceptPromise = this._session
...
class BidiRequest {
  constructor(frame, redirectedFrom, payload, route) {
    ...
    this.request = new network.Request(
      ...
      (_payload$navigation = payload.navigation) !== null && _payload$navigation !== void 0
        ? _payload$navigation
        : undefined,
      payload.request.url,
      "other",
