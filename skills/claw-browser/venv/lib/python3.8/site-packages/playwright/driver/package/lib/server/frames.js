...
class FrameManager {
  ...
  requestStarted(request, route) {
    ...
    if (route) {
      var _this$_page$_serverRe,
        _this$_page,
        _this$_page$_clientRe,
        _this$_page2,
        _this$_page$_browserC,
        _this$_page$_browserC2;
      const r = new network.Route(request, route);
      if (
        (_this$_page$_serverRe = (_this$_page = this._page)._serverRequestInterceptor) !== null &&
        _this$_page$_serverRe !== void 0 &&
        _this$_page$_serverRe.call(_this$_page, r, request)
      )
        return;
      if (
        (_this$_page$_clientRe = (_this$_page2 = this._page)._clientRequestInterceptor) !== null &&
        _this$_page$_clientRe !== void 0 &&
        _this$_page$_clientRe.call(_this$_page2, r, request)
      )
        return;
      if (
        (_this$_page$_browserC = (_this$_page$_browserC2 = this._page._browserContext)
          ._requestInterceptor) !== null &&
        _this$_page$_browserC !== void 0 &&
        _this$_page$_browserC.call(_this$_page$_browserC2, r, request)
      )
        return;
      r.continue({
        isFallback: true,
...
class Frame extends _instrumentation.SdkObject {
  ...
  async raceNavigationAction(progress, options, action) {
    return _utils.LongStandingScope.raceMultiple(
  ...
  redirectNavigation(url, documentId, referer) {
    const controller = new _progress.ProgressController(
      (0, _instrumentation.serverSideCallMetadata)(),
      this,
    );
    const data = {
      url,
  ...
  async _goto(progress, url, options) {
    return this.raceNavigationAction(progress, options, async () =>
      this._gotoAction(progress, url, options),
    );
  }
  async _gotoAction(progress, url, options) {
    ...
    const navigateResult = await this._page._delegate
      .navigateFrame(this, url, referer)
      .finally(() => this.off(Frame.Events.InternalNavigation, collectNavigations));
    let event;
    if (navigateResult.newDocumentId) {
      const predicate = (event) => {
        ...
        return (
          event.newDocument &&
          (event.newDocument.documentId === navigateResult.newDocumentId || !event.error)
        );
      };
  ...
  async evaluateExpression(expression, options = {}, arg) {
    ...
    const context = await this._context(
      (_options$world = options.world) !== null && _options$world !== void 0
        ? _options$world
        : "main",
    );
  ...
  async evaluateExpressionHandle(expression, options = {}, arg) {
    var _options$world2;
    const context = await this._context(
      (_options$world2 = options.world) !== null && _options$world2 !== void 0
        ? _options$world2
        : "main",
    );
  ...
  async waitForSelectorInternal(
    progress,
    selector,
    performLocatorHandlersCheckpoint,
    options,
    scope,
  ) {
    const { state = "visible" } = options;
    const promise = this.retryWithProgressAndTimeouts(
      ...
      async (continuePolling) => {
        ...
        const { log, visible, attached } = await result.evaluate((r) => ({
          log: r.log,
          visible: r.visible,
          attached: r.attached,
        }));
        ...
        const element =
          state === "attached" || state === "visible"
            ? await result.evaluateHandle((r) => r.element)
            : null;
        result.dispose();
  ...
  async evalOnSelector(selector, strict, expression, isFunction, arg, scope) {
    const handle = await this.selectors.query(
      selector,
      {
        strict,
      },
  ...
  origin() {
    ...
    return (_network$parsedURL = network.parsedURL(this._url)) === null ||
      _network$parsedURL === void 0
      ? void 0
      : _network$parsedURL.origin;
  }
  parentFrame() {
    ...
    const errorPromise = new Promise((resolve) => {
      listeners.push(
        _eventsHelper.eventsHelper.addEventListener(
          this._page._browserContext,
          _browserContext.BrowserContext.Events.Console,
          (message) => {
  ...
  async retryWithProgressAndTimeouts(progress, timeouts, action) {
    ...
    while (progress.isRunning()) {
      ...
      if (timeout) {
        ...
        await _utils.LongStandingScope.raceMultiple(
  ...
  async _retryWithProgressIfNotConnected(
    ...
    return this.retryWithProgressAndTimeouts(
      ...
      async (continuePolling) => {
        ...
        const { log, success } = await result.evaluate((r) => ({
          log: r.log,
          success: r.success,
        }));
        if (log) progress.log(log);
  ...
  async rafrafTimeoutScreenshotElementWithProgress(progress, selector, timeout, options) {
    return await this._retryWithProgressIfNotConnected(
      ...
      selector,
      true /* strict */,
      true /* performLocatorHandlersCheckpoint */,
      async (handle) => {
    ...
    return controller.run(async (progress) => {
      return dom.assertDone(
        await this._retryWithProgressIfNotConnected(
          ...
          selector,
          options.strict,
          !options.force /* performLocatorHandlersCheckpoint */,
          (handle) =>
            handle._click(progress, {
              ...options,
    ...
    await controller.run(async (progress) => {
      ...
      dom.assertDone(
        await this._retryWithProgressIfNotConnected(
          ...
          target,
          options.strict,
          false /* performLocatorHandlersCheckpoint */,
          async (handle) => {
            return handle._retryPointerAction(
              progress,
              "move and up",
  ...
  async isVisibleInternal(selector, options = {}, scope) {
    try {
      ...
      if (
        ...
        (0, _selectorParser.isInvalidSelectorError)(e) ||
  ...
  async _expectImpl(metadata, selector, options) {
    ...
    try {
      ...
      try {
        const resultOneShot = await new _progress.ProgressController(metadata, this).run(
          async (progress) => {
      ...
      return await new _progress.ProgressController(metadata, this).run(async (progress) => {
        return await this.retryWithProgressAndTimeouts(
          progress,
          [100, 250, 500, 1000],
          async (continuePolling) => {
            await this._page.performLocatorHandlersCheckpoint(progress);
  ...
  async _expectInternal(progress, selector, options, lastIntermediateResult) {
    ...
    const world =
      options.expression === "to.have.property"
        ? "main"
        : (_info$world = info === null || info === void 0 ? void 0 : info.world) !== null &&
            _info$world !== void 0
          ? _info$world
          : "utility";
    ...
    progress.throwIfAborted();
    const { log, matches, received, missingReceived } = await injected.evaluate(
      async (injected, { info, options, callId }) => {
    ...
    return controller.run(async (progress) => {
      return this.retryWithProgressAndTimeouts(progress, [100], async () => {
        ...
        const handle = await injectedScript.evaluateHandle(
          (injected, { expression, isFunction, polling, arg }) => {
            const predicate = () => {
    ...
    return controller.run(async (progress) => {
      ...
      const promise = this.retryWithProgressAndTimeouts(
        ...
        async (continuePolling) => {
          ...
          progress.throwIfAborted();
          if (!resolved) return continuePolling;
          const { log, success, value } = await resolved.injected.evaluate(
            (injected, { info, callbackText, taskData, callId, root }) => {
    ...
    await context
      .evaluate(
        async ({ ls }) => {
          ...
          // Clean IndexedDB
          for (const db of (await ((_indexedDB$databases = (_indexedDB = indexedDB).databases) ===
            null || _indexedDB$databases === void 0
            ? void 0
            : _indexedDB$databases.call(_indexedDB))) || []) {
